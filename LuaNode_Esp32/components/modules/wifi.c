...
static int wifi_setphymode( lua_State* L )
{
  ...
  if ( mode != PHY_MODE_11B && mode != PHY_MODE_11G && mode != PHY_MODE_11N )
    return luaL_error( L, "wrong arg type" );
  wifi_set_phy_mode(STATION_IF, (uint8_t)mode);
  lua_pushinteger( L, mode );
  return 1;
}
...
static int wifi_getphymode( lua_State* L )
{
  ...
  bool err = wifi_get_phy(STATION_IF, &config);
  lua_pushinteger( L, config.phy_mode );
  return 1;
#endif
	return 0;
}
...
///////////////////////////////////////////////////
static unsigned char get_one_char(const char c) 
{
	if(c>'0' && c<'9') return c;
	if(c>='a' && c<='f') {
		return c-'a'+10;
...
static void str2hex(char *des, const char* src)
{
	char high = *src, low = *(src+1);
	uint8_t res = 0;
	res = res | get_one_char(low);
	res = res | (get_one_char(high)<<4);
	*des = res;
}

static void ets_str2macaddr(char *des, const char *src)
{
	int i;
	for (i=0; i<6; i++) 
		str2hex(des+i*2, src+i*3);
}
//////////////////////////////////////////////////
...
LUANODE_MODULE(WIFI, "wifi", wifi_map, NULL);
