...
{
    BLE_HS_LOG(DEBUG, "addr=");
    ble_hs_log_flat_buf(sec->peer_addr.val, 6);
    BLE_HS_LOG(DEBUG, "peer_addr_type=%d ", sec->peer_addr.type);
...
static void 
ble_store_config_print_value_cccd(const struct ble_store_value_cccd*sec)
{
    BLE_HS_LOG(DEBUG, "addr=");
    ble_hs_log_flat_buf(sec->peer_addr.val, 6);
    BLE_HS_LOG(DEBUG, " ");
    ...
    BLE_HS_LOG(DEBUG, "peer_addr_type=%d ", sec->peer_addr.type);
    BLE_HS_LOG(DEBUG, "chr_val_handle=%d(0x%04x) ", sec->chr_val_handle,sec->chr_val_handle); 
    BLE_HS_LOG(DEBUG, "flags=%d(0x%04x) ", sec->flags,sec->flags); 
    BLE_HS_LOG(DEBUG, "valuechanged=%d(0x%04x) ", sec->value_changed,sec->value_changed);
    BLE_HS_LOG(DEBUG, "\n");    
}
...
static void
ble_store_config_print_key_cccd(const struct ble_store_key_cccd *key_ccd)
{
    if (ble_addr_cmp(&key_ccd->peer_addr, BLE_ADDR_ANY)) {
        BLE_HS_LOG(DEBUG, "peer_addr_type=%d peer_addr=",
                       key_ccd->peer_addr.type);
        ble_hs_log_flat_buf(key_ccd->peer_addr.val, 6);
        BLE_HS_LOG(DEBUG, " ");
    }

    BLE_HS_LOG(DEBUG, "%d(0x%04x) %d(0x%04x)\r\n",
                       key_ccd->chr_val_handle,key_ccd->chr_val_handle,key_ccd->idx,key_ccd->idx);
...
ble_store_config_write_cccd(const struct ble_store_value_cccd *value_cccd)
{
    ...
    idx = ble_store_config_find_cccd(&key_cccd);
    if (idx == -1) {
        if (ble_store_config_num_cccds >= (MYNEWT_VAL(BLE_STORE_MAX_CCCDS)*MYNEWT_VAL(BLE_STORE_MAX_BONDS))) {
                      ...
                      union ble_store_value *value)
{
    ...
    case BLE_STORE_OBJ_TYPE_CCCD:
        BLE_HS_LOG(DEBUG, "looking up xxxx ccd; ");
        ble_store_config_print_key_cccd(&key->cccd);
        BLE_HS_LOG(DEBUG, "\n");        
