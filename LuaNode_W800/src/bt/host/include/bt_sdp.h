...
typedef void (*btsdp_search_callback)(tls_bt_status_t status, tls_bt_addr_t *bd_addr, uint8_t *uuid, int num_records, bluetooth_sdp_record *records);
...
typedef struct
{
    ...
    tls_bt_status_t (*init)(btsdp_callbacks_t *callbacks);

    /** Unregister BT SDP */
    tls_bt_status_t (*deinit)();

    /** Search for SDP records with specific uuid on remote device */
    tls_bt_status_t (*sdp_search)(tls_bt_addr_t *bd_addr,  const uint8_t *uuid);
    ...
    tls_bt_status_t (*remove_sdp_record)(int sdp_handle);
} btsdp_interface_t;
