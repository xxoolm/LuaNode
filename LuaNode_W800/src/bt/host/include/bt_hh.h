 ...
 */
typedef void (* bthh_virtual_unplug_callback)(tls_bt_addr_t *bd_addr, bthh_status_t hh_status);

/** Callback for get hid info
 *  hid_info will contain attr_mask, sub_class, app_id, vendor_id, product_id, version, ctry_code, len
 */
typedef void (* bthh_hid_info_callback)(tls_bt_addr_t *bd_addr, bthh_hid_info_t hid_info);
 ...
 */
typedef void (* bthh_handshake_callback)(tls_bt_addr_t *bd_addr, bthh_status_t hh_status);
...
typedef struct
{
     ...
     */
    tls_bt_status_t (*init)(bthh_callbacks_t *callbacks);
    ...
    tls_bt_status_t (*virtual_unplug)(tls_bt_addr_t *bd_addr);

    /** Set the HID device descriptor for the specified HID device. */
    tls_bt_status_t (*set_info)(tls_bt_addr_t *bd_addr, bthh_hid_info_t hid_info);

    /** Get the HID proto mode. */
    tls_bt_status_t (*get_protocol)(tls_bt_addr_t *bd_addr, bthh_protocol_mode_t protocolMode);
    ...
    tls_bt_status_t (*send_data)(tls_bt_addr_t *bd_addr, char *data);
