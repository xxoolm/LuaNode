...
typedef struct
{
     ...
     */
    tls_bt_status_t (*init)(bthf_client_callbacks_t *callbacks);
    ...
    tls_bt_status_t (*start_voice_recognition)(void);

    /** stop voice recognition */
    tls_bt_status_t (*stop_voice_recognition)(void);

    /** volume control */
    tls_bt_status_t (*volume_control)(bthf_client_volume_type_t type, int volume);

    /** place a call with number a number
     * if number is NULL last called number is called (aka re-dial)*/
    tls_bt_status_t (*dial)(const char *number);

    /** place a call with number specified by location (speed dial) */
    tls_bt_status_t (*dial_memory)(int location);

    /** perform specified call related action
     * idx is limited only for enhanced call control related action
     */
    tls_bt_status_t (*handle_call_action)(bthf_client_call_action_t action, int idx);

    /** query list of current calls */
    tls_bt_status_t (*query_current_calls)(void);

    /** query name of current selected operator */
    tls_bt_status_t (*query_current_operator_name)(void);

    /** Retrieve subscriber information */
    tls_bt_status_t (*retrieve_subscriber_info)(void);

    /** Send DTMF code*/
    tls_bt_status_t (*send_dtmf)(char code);

    /** Request a phone number from AG corresponding to last voice tag recorded */
    tls_bt_status_t (*request_last_voice_tag_number)(void);

    /** Closes the interface. */
    void (*cleanup)(void);

    /** Send AT Command. */
    tls_bt_status_t (*send_at_cmd)(int cmd, int val1, int val2, const char *arg);
