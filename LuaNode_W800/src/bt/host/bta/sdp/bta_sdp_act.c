...
static void bta_sdp_search_cback(uint16_t result, void *user_data)
{
    ...
    GKI_freebuf(user_data);   // We no longer need the user data to track the search
}
...
void bta_sdp_search(tBTA_SDP_MSG *p_data)
{
    ...
    tBT_UUID *bta_sdp_search_uuid = GKI_getbuf(sizeof(tBT_UUID));
    wm_memcpy(bta_sdp_search_uuid, &(p_data->get_search.uuid), sizeof(tBT_UUID));
