...
static void bta_av_st_rc_timer(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
            ...
            (p_scb->use_rc == TRUE || (p_scb->role & BTA_AV_ROLE_AD_ACP)))
    {
        if((p_scb->wait & BTA_AV_WAIT_ROLE_SW_BITS) == 0)
        {
            ...
            #else
            p_scb->avrc_ct_timer.p_cback = NULL;
            p_scb->avrc_ct_timer.data = p_scb->hndl;
            bta_sys_start_timer(&p_scb->avrc_ct_timer, BTA_AV_RC_DISC_TIME_VAL,
                                BTA_AV_AVRC_TIMER_EVT, p_scb->hndl);
            #endif
        }
...
**
*******************************************************************************/
static uint8_t bta_av_next_getcap(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    int     i;
    tAVDT_GETCAP_REQ    *p_req;
    uint8_t     sent_cmd = FALSE;
...
static void bta_av_proc_stream_evt(uint8_t handle, BD_ADDR bd_addr, uint8_t event, tAVDT_CTRL *p_data, int index)
{
    ...
    if(p_data)
    {
        if(event == AVDT_SECURITY_IND_EVT)
        {
            ...
            if(event == AVDT_SECURITY_CFM_EVT && p_data->hdr.err_code == 0)
            {
    ...
    if(p_scb)
    {
        tBTA_AV_STR_MSG *p_msg =
                        (tBTA_AV_STR_MSG *)GKI_getbuf(sizeof(tBTA_AV_STR_MSG) + sec_len);
        ...
        if(p_data != NULL)
        {
            wm_memcpy(&p_msg->msg, p_data, sizeof(tAVDT_CTRL));
            ...
            switch(event)
            {
                ...
                case AVDT_CONFIG_IND_EVT:
                    ...
                    wm_memcpy(&p_msg->cfg, p_data->config_ind.p_cfg, sizeof(tAVDT_CFG));
                    break;

                case AVDT_SECURITY_IND_EVT:
                    p_msg->msg.security_ind.p_data = (uint8_t *)(p_msg + 1);
                    wm_memcpy(p_msg->msg.security_ind.p_data, p_data->security_ind.p_data, sec_len);
                    break;
                ...
                case AVDT_SECURITY_CFM_EVT:
                    ...
                    {
                        wm_memcpy(p_msg->msg.security_cfm.p_data, p_data->security_cfm.p_data, sec_len);
                    }
...
**
*******************************************************************************/
static void bta_av_a2d_sdp_cback(uint8_t found, tA2D_Service *p_service)
{
    ...
    tBTA_AV_SDP_RES *p_msg =
                    (tBTA_AV_SDP_RES *)GKI_getbuf(sizeof(tBTA_AV_SDP_RES));
            ...
            if(p_scb->wait & BTA_AV_WAIT_ROLE_SW_RES_OPEN)
            {
                ...
                if(p_data->role_res.hci_status != HCI_SUCCESS)
                {
                    ...
                    if(p_scb->seps[p_scb->sep_idx].tsep == AVDT_TSEP_SRC)
                    {
                        ...
                        if(p_scb->seps[p_scb->sep_idx].tsep == AVDT_TSEP_SNK)
                        {
...
void bta_av_do_disc_a2d(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    wm_memcpy(&(p_scb->open_api), &(p_data->api_open), sizeof(tBTA_AV_API_OPEN));
    ...
    switch(p_data->api_open.switch_res)
    {
        case BTA_AV_RS_NONE:
            if(bta_av_switch_if_needed(p_scb) || !bta_av_link_role_ok(p_scb, A2D_SET_MULTL_BIT))
            {
                ...
                wm_memcpy(&p_scb->q_info.open, &p_data->api_open, sizeof(tBTA_AV_API_OPEN));
                p_scb->wait |= BTA_AV_WAIT_ROLE_SW_RES_OPEN;
    ...
    if(p_scb->wait & BTA_AV_WAIT_CHECK_RC)
    {
        ...
        #else
        p_scb->avrc_ct_timer.p_cback = NULL;
        p_scb->avrc_ct_timer.data = (TIMER_PARAM_TYPE)p_scb->hndl;
        bta_sys_start_timer(&p_scb->avrc_ct_timer, BTA_AV_RC_DISC_TIME_VAL,
                            BTA_AV_AVRC_TIMER_EVT, p_scb->hndl);
        #endif
    }
...
void bta_av_cleanup(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    /* free any buffers */
    GKI_free_and_reset_buf((void **)&p_scb->p_cap);
    ...
    #else
    bta_sys_stop_timer(&p_scb->avrc_ct_timer);
    #endif
    #if 0
...
void bta_av_config_ind(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    wm_memcpy(p_scb->cfg.codec_info, p_evt_cfg->codec_info, AVDT_CODEC_SIZE);
    ...
    #else
    bta_sys_stop_timer(&bta_av_cb.accept_signalling_timer);
    #endif
...
void bta_av_disconnect_req(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    alarm_cancel(p_scb->avrc_ct_timer);
    #else
    bta_sys_stop_timer(&bta_av_cb.link_signalling_timer);
    bta_sys_stop_timer(&p_scb->avrc_ct_timer);
    #endif
...
void bta_av_setconfig_rsp(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
            ...
            (p_scb->seps[p_scb->sep_idx].p_app_data_cback != NULL))
    {
        ...
        wm_memcpy(av_sink_codec_info.avk_config.bd_addr, p_scb->peer_addr, sizeof(BD_ADDR));
...
void bta_av_str_opened(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    wm_memset(&p_scb->q_info, 0, sizeof(tBTA_AV_Q_INFO));
    p_scb->l2c_bufs = 0;
...
void bta_av_do_close(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    #else
    p_scb->avrc_ct_timer.p_cback = (TIMER_CBACK *)NULL;
    p_scb->avrc_ct_timer.data = (TIMER_PARAM_TYPE)p_scb->hndl;
    bta_sys_start_timer(&p_scb->avrc_ct_timer, BTA_AV_CLOSE_REQ_TIME_VAL,
                        BTA_AV_API_CLOSE_EVT, p_scb->hndl);
    #endif
...
void bta_av_getcap_results(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    wm_memcpy(&cfg, &p_scb->cfg, sizeof(tAVDT_CFG));
    cfg.num_codec = 1;
    cfg.num_protect = p_scb->p_cap->num_protect;
    wm_memcpy(cfg.codec_info, p_scb->p_cap->codec_info, AVDT_CODEC_SIZE);
    wm_memcpy(cfg.protect_info, p_scb->p_cap->protect_info, AVDT_PROTECT_SIZE);
                                  ...
                                  &cfg.num_protect, cfg.protect_info) == 0))
    {
        ...
        wm_memcpy(&p_scb->cfg, &cfg, sizeof(tAVDT_CFG));
...
void bta_av_reconfig(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    bta_sys_stop_timer(&p_scb->avrc_ct_timer);
    #endif
    wm_memcpy(p_cfg, &p_scb->cfg, sizeof(tAVDT_CFG));
    p_cfg->num_protect = p_rcfg->num_protect;
    wm_memcpy(p_cfg->codec_info, p_rcfg->codec_info, AVDT_CODEC_SIZE);
    wm_memcpy(p_cfg->protect_info, p_rcfg->p_protect_info, p_rcfg->num_protect);
...
void bta_av_start_ok(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    if(new_role & BTA_AV_ROLE_START_INT)
    {
        ...
        if((new_role & BTA_AV_ROLE_AD_ACP) && (new_role & BTA_AV_ROLE_SUSPEND_OPT))
        {
            suspend = TRUE;
        }
...
void bta_av_rcfg_cfm(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    */
    ...
    else
    {
        ...
        wm_memcpy(p_scb->cfg.codec_info, p_data->str_msg.msg.reconfig_cfm.p_cfg->codec_info, AVDT_CODEC_SIZE);
...
void bta_av_rcfg_open(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    else
    {
        ...
        wm_memcpy(p_scb->cfg.codec_info, p_scb->p_cap->codec_info, AVDT_CODEC_SIZE);
...
void bta_av_open_at_inc(tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    else
    {
        ...
        wm_memcpy(p_buf, &(p_scb->open_api), sizeof(tBTA_AV_API_OPEN));
        p_scb->skip_sdp = TRUE;
    ...
    if(p_scb->started != TRUE)
    {
                ...
                p_scb->chnl == BTA_AV_CHNL_AUDIO)
        {
                                        ...
                                        &a2dp_offload_start.remote_cid, &a2dp_offload_start.lm_handle))
            {
                ...
                wm_memcpy(a2dp_offload_start.codec_info, p_scb->cfg.codec_info,
