...
static void bta_av_rc_ctrl_cback(uint8_t handle, uint8_t event, uint16_t result, BD_ADDR peer_addr)
{
    ...
    if(msg_event)
    {
        tBTA_AV_RC_CONN_CHG *p_msg =
                        (tBTA_AV_RC_CONN_CHG *)GKI_getbuf(sizeof(tBTA_AV_RC_CONN_CHG));
...
static void bta_av_rc_msg_cback(uint8_t handle, uint8_t label, uint8_t opcode, tAVRC_MSG *p_msg)
{
    ...
    if(opcode == AVRC_OP_VENDOR && p_msg->vendor.p_vendor_data != NULL)
    {
        ...
        if(opcode == AVRC_OP_PASS_THRU && p_msg->pass.p_pass_data != NULL)
        {
    ...
    tBTA_AV_RC_MSG *p_buf =
                    (tBTA_AV_RC_MSG *)GKI_getbuf(sizeof(tBTA_AV_RC_MSG) + data_len);
    p_buf->hdr.event = BTA_AV_AVRC_MSG_EVT;
    p_buf->handle = handle;
    p_buf->label = label;
    p_buf->opcode = opcode;
    wm_memcpy(&p_buf->msg, p_msg, sizeof(tAVRC_MSG));

    /* Copy the data payload, and set the pointer to it */
    if(p_data_src != NULL)
    {
        uint8_t *p_data_dst = (uint8_t *)(p_buf + 1);
        wm_memcpy(p_data_dst, p_data_src, data_len);
...
**
*******************************************************************************/
static tBTA_AV_CODE bta_av_group_navi_supported(uint8_t len, uint8_t *p_data, uint8_t is_inquiry)
{
...
**
*******************************************************************************/
static tBTA_AV_CODE bta_av_op_supported(tBTA_AV_RC rc_id, uint8_t is_inquiry)
{
    ...
    if(p_bta_av_rc_id)
    {
        ...
        else
        {
            if(p_bta_av_rc_id[rc_id >> 4] & (1 << (rc_id & 0x0F)))
            {
                ...
                if((p_bta_av_cfg->rc_pass_rsp == BTA_AV_RSP_INTERIM) && p_bta_av_rc_id_ac)
                {
...
void bta_av_rc_meta_rsp(tBTA_AV_CB *p_cb, tBTA_AV_DATA *p_data)
{
    ...
    if(do_free)
    {
        GKI_free_and_reset_buf((void **)&p_data->api_meta_rsp.p_pkt);
    }
}
...
tBTA_AV_EVT bta_av_proc_meta_cmd(tAVRC_RESPONSE  *p_rc_rsp, tBTA_AV_RC_MSG *p_msg, uint8_t *p_ctype)
{
    ...
    *p_ctype = AVRC_RSP_REJ;
    ...
    if(p_vendor->hdr.subunit_type != AVRC_SUB_PANEL)
    {
        ...
        if(!AVRC_IsValidAvcType(pdu, p_vendor->hdr.ctype))
        {
                    ...
                    else
                    {
                        ...
                        if(u8 == AVRC_CAP_COMPANY_ID)
                        {
                            *p_ctype = AVRC_RSP_IMPL_STBL;
                            ...
                            wm_memcpy(p_rc_rsp->get_caps.param.company_id, p_bta_av_cfg->p_meta_co_ids,
                                      (p_bta_av_cfg->num_co_ids << 2));
                        }
                        else
                            if(u8 == AVRC_CAP_EVENTS_SUPPORTED)
                            {
                                *p_ctype = AVRC_RSP_IMPL_STBL;
                                p_rc_rsp->get_caps.count = p_bta_av_cfg->num_evt_ids;
                                wm_memcpy(p_rc_rsp->get_caps.param.event_id, p_bta_av_cfg->p_meta_evt_ids,
...
void bta_av_rc_msg(tBTA_AV_CB *p_cb, tBTA_AV_DATA *p_data)
{
    ...
    if(p_data->rc_msg.opcode == AVRC_OP_PASS_THRU)
    {
          ...
          )
        {
            ...
            const char *avrcp_ct_support = "true";
            ...
            if(p_data->rc_msg.msg.pass.op_id == AVRC_ID_VENDOR)
            {
                ...
                if(((p_data->rc_msg.msg.pass.op_id == AVRC_ID_VOL_UP) ||
            ...
            if(p_data->rc_msg.msg.hdr.ctype == BTA_AV_RSP_ACCEPT || p_data->rc_msg.msg.hdr.ctype == BTA_AV_RSP_INTERIM)
            {
                ...
                wm_memcpy(&av.remote_cmd.hdr, &p_data->rc_msg.msg.hdr, sizeof(tAVRC_HDR));
        ...
        /* id response type is not impl, we have to release label */
            ...
            if(p_data->rc_msg.msg.hdr.ctype >= AVRC_RSP_NOT_IMPL)
            {
                /* set up for callback */
                        ...
                        (p_data->rc_msg.msg.pass.pass_len > 0))
                {
                    av.remote_rsp.p_data =
                                    (uint8_t *)GKI_getbuf(p_data->rc_msg.msg.pass.pass_len);
                    APPL_TRACE_DEBUG("Vendor Unique data len = %d",
                                     p_data->rc_msg.msg.pass.pass_len);
                    wm_memcpy(av.remote_rsp.p_data, p_data->rc_msg.msg.pass.p_pass_data,
                              p_data->rc_msg.msg.pass.pass_len);
                }
            }
    ...
    /* else if this is a vendor specific command or response */
        ...
        if(p_data->rc_msg.opcode == AVRC_OP_VENDOR)
        {
            /* set up for callback */
...
**
*******************************************************************************/
void bta_av_stream_chg(tBTA_AV_SCB *p_scb, uint8_t started)
{
    uint8_t   started_msk;
    int     i;
    uint8_t   *p_streams;
    uint8_t no_streams = FALSE;
    tBTA_AV_SCB *p_scbi;
...
void bta_av_disable(tBTA_AV_CB *p_cb, tBTA_AV_DATA *p_data)
{
    ...
    p_cb->disabling = TRUE;
    bta_av_close_all_rc(p_cb);
    GKI_free_and_reset_buf((void **)&p_cb->p_disc_db);
                ...
                if((!(mask & p_cb->conn_lcb)) && (p_cb->p_scb[xx] != NULL))
                {
                    ...
                    if(p_data->hdr.offset == AVDT_ACP)
                    {
                        ...
                        alarm_set_on_queue(p_cb->accept_signalling_timer,
                                           ...
                                           btu_bta_alarm_queue);
                        #else
                        p_cb->accept_signalling_timer.p_cback = (TIMER_CBACK *)&bta_av_accept_signalling_timer_cback;
                        p_cb->accept_signalling_timer.param = (TIMER_PARAM_TYPE)UINT_TO_PTR(xx);
                        bta_sys_start_timer(&p_cb->accept_signalling_timer, BTA_AV_ACCEPT_SIGNALLING_TIMEOUT_MS, 0, 0);
                        #endif
                    }
    ...
    for(xx = 0; xx < BTA_AV_NUM_LINKS; xx++)
    {
        ...
        if(mask & p_cb->conn_lcb)
        {
            ...
            if(!p_lcb->conn_msk)
            {
                ...
                #else
                p_cb->link_signalling_timer.p_cback = (TIMER_CBACK *)NULL;
                p_cb->link_signalling_timer.data = (TIMER_PARAM_TYPE)0;
                bta_sys_start_timer(&p_cb->link_signalling_timer,
                                    BTA_AV_SIGNALLING_TIMEOUT_MS,
                                    BTA_AV_SIGNALLING_TIMER_EVT, 0);
                #endif
        ...
        if(p_scb->coll_mask & BTA_AV_COLL_INC_TMR)
        {
            ...
            if(bta_av_is_scb_opening(p_scb))
            {
                if(p_scb->sdp_discovery_started)
                {
                    ...
                    #else
                    p_cb->accept_signalling_timer.p_cback = (TIMER_CBACK *)&bta_av_accept_signalling_timer_cback;
                    p_cb->accept_signalling_timer.param = (TIMER_PARAM_TYPE)UINT_TO_PTR(inx);
                    bta_sys_start_timer(&p_cb->accept_signalling_timer, BTA_AV_ACCEPT_SIGNALLING_TIMEOUT_MS, 0, 0);
                    #endif
                }
                else
                ...
                if(bta_av_is_scb_incoming(p_scb))
                {
...
tBTA_AV_FEAT bta_avk_check_peer_features(uint16_t service_uuid)
{
    ...
    while(p_rec)
    {
        ...
        if((SDP_FindAttributeInRec(p_rec, ATTR_ID_BT_PROFILE_DESC_LIST)) != NULL)
        {
            /* get profile version (if failure, version parameter is not updated) */
            uint16_t peer_rc_version = 0;
            uint8_t val = SDP_FindProfileVersionInRec(
    ...
    for(i = 0; i < BTA_AV_NUM_RCB; i++)
    {
        ...
        if(p_rcb->handle == p_msg->handle)
        {
            ...
            if(p_rcb->shdl)
            {
                ...
                if(p_rcb->lidx == (BTA_AV_NUM_LINKS + 1))
                {
...
void bta_av_dereg_comp(tBTA_AV_DATA *p_data)
{
    ...
    if(p_scb)
    {
        ...
        bta_sys_stop_timer(&p_scb->avrc_ct_timer);
        #endif
        GKI_free_and_reset_buf((void **)&p_cb->p_scb[p_scb->hdi]);
    }
