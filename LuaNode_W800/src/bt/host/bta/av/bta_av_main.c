...
static void bta_av_api_enable(tBTA_AV_DATA *p_data)
{
    ...
    wm_memset(&bta_av_cb, 0, sizeof(tBTA_AV_CB));
...
void bta_av_conn_cback(uint8_t handle, BD_ADDR bd_addr, uint8_t event, tAVDT_CTRL *p_data)
{
    ...
    #endif
    {
        ...
        tBTA_AV_STR_MSG *p_msg =
                        (tBTA_AV_STR_MSG *)GKI_getbuf(sizeof(tBTA_AV_STR_MSG));
        p_msg->hdr.event = evt;
...
static void bta_av_api_register(tBTA_AV_DATA *p_data)
{
    ...
    tBTA_UTL_COD    cod;
    uint8_t           index = 0;
    wm_memset(&cs, 0, sizeof(tAVDT_CS));
    ...
    do
    {
        ...
        /* wm_memset(&cs.cfg,0,sizeof(tAVDT_CFG)); */
        cs.cfg.num_codec = 1;
        /*
         * wm_memset of cs takes care setting call back pointers to null.
        ...
        */
        ...
        if(registr.chnl == BTA_AV_CHNL_AUDIO)
        {
            ...
            wm_memcpy(&p_scb->cfg, &cs.cfg, sizeof(tAVDT_CFG));
...
**
*******************************************************************************/
uint8_t bta_av_chk_start(tBTA_AV_SCB *p_scb)
{
    uint8_t start = FALSE;
    tBTA_AV_SCB *p_scbi;
    int i;
    ...
    for(i = 0; i < BTA_AV_NUM_STRS; i++)
    {
        ...
        if(p_scb && (bdcmp(peer_addr, p_scb->peer_addr) == 0))
        {
            tBTA_AV_ROLE_RES *p_buf =
                            (tBTA_AV_ROLE_RES *)GKI_getbuf(sizeof(tBTA_AV_ROLE_RES));
...
**
*******************************************************************************/
uint8_t bta_av_switch_if_needed(tBTA_AV_SCB *p_scb)
{
    uint8_t role;
...
**
*******************************************************************************/
uint8_t bta_av_link_role_ok(tBTA_AV_SCB *p_scb, uint8_t bits)
{
    uint8_t role;
    uint8_t is_ok = TRUE;
...
void bta_av_dup_audio_buf(tBTA_AV_SCB *p_scb, BT_HDR *p_buf)
{
    ...
    APPL_TRACE_DEBUG("bta_av_dup_audio_buf\r\n");
    ...
    for(int i = 0; i < BTA_AV_NUM_STRS; i++)
    {
        ...
        /* Enqueue the data */
        BT_HDR *p_new = (BT_HDR *)GKI_getbuf(copy_size);
        wm_memcpy(p_new, p_buf, copy_size);
...
**
*******************************************************************************/
uint8_t bta_av_hdl_event(BT_HDR *p_msg)
{
    ...
    APPL_TRACE_EVENT("bta_av_hdl_event, event=%d, first_event=%d\r\n", event, BTA_AV_FIRST_NSM_EVT);
    ...
    if(event >= first_event)
    {
        ...
        if(event >= BTA_AV_FIRST_SM_EVT && event <= BTA_AV_LAST_SM_EVT)
        {
            #if (defined(BTA_AV_DEBUG) && BTA_AV_DEBUG == TRUE)
...
char *bta_av_evt_code(uint16_t evt_code)
{
    switch(evt_code)
    {
        ...
        case BTA_AV_API_SINK_ENABLE_EVT:
            return "SINK_ENABLE";
            #endif
            #if (AVDT_REPORTING == TRUE)
