...
extern uint8_t bta_gattc_sm_execute(tBTA_GATTC_CLCB *p_clcb, uint16_t event, tBTA_GATTC_DATA *p_data);
...
extern uint8_t bta_gattc_uuid_compare(const tBT_UUID *p_src, const tBT_UUID *p_tar, uint8_t is_precise);
extern uint8_t bta_gattc_check_notif_registry(tBTA_GATTC_RCB  *p_clreg, tBTA_GATTC_SERV *p_srcb, tBTA_GATTC_NOTIFY  *p_notify);
extern uint8_t bta_gattc_mark_bg_conn(tBTA_GATTC_IF client_if,  BD_ADDR_PTR remote_bda, uint8_t add, uint8_t is_listen);
...
extern void bta_gattc_get_gatt_db(uint16_t conn_id, uint16_t start_handle, uint16_t end_handle, tls_btgatt_db_element_t **db, int *count);
