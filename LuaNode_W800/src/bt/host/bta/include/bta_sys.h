...
typedef uint8_t (tBTA_SYS_VS_EVT_HDLR)(uint16_t evt, void *p);

/* event handler function type */
typedef uint8_t (tBTA_SYS_EVT_HDLR)(BT_HDR *p_msg);
...
extern uint8_t bta_sys_is_register(uint8_t id);
extern uint16_t bta_sys_get_sys_features(void);
extern void bta_sys_sendmsg(void *p_msg);
extern void bta_sys_start_timer(TIMER_LIST_ENT *p_tle, uint64_t interval,
                                uint16_t event, uint16_t layer_specific);
extern void bta_sys_stop_timer(TIMER_LIST_ENT *p_tle);
extern uint32_t bta_sys_get_timer_remaining_ms(TIMER_LIST_ENT *p_tle);
