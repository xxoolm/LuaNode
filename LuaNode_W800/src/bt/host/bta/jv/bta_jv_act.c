...
static void fcchan_conn_chng_cbk(uint16_t chan, BD_ADDR bd_addr, uint8_t connected,
                                 uint16_t reason, tBT_TRANSPORT);
static void fcchan_data_cbk(uint16_t chan, BD_ADDR bd_addr, BT_HDR *p_buf);


extern void uuid_to_string_legacy(tls_bt_uuid_t *p_uuid, char *str);
static inline void logu(const char *title, const uint8_t *p_uuid)
{
    char uuids[128];
    uuid_to_string_legacy((tls_bt_uuid_t *)p_uuid, uuids);
...
**
*******************************************************************************/
static void bta_jv_clear_pm_cb(tBTA_JV_PM_CB *p_pm_cb, uint8_t close_conn)
{
...
{
    uint8_t bRfcHandle = (jv_handle & BTA_JV_RFCOMM_MASK) != 0;
...
**
*******************************************************************************/
uint8_t bta_jv_check_psm(uint16_t psm)
{
    uint8_t ret = FALSE;
...
void bta_jv_enable(tBTA_JV_MSG *p_data)
{
    ...
    wm_memset(bta_jv_cb.free_psm_list, 0, sizeof(bta_jv_cb.free_psm_list));
}
...
static void bta_jv_set_free_psm(uint16_t psm)
{
    ...
    for(int i = 0; i < cnt; i++)
    {
        if(bta_jv_cb.free_psm_list[i] == 0)
        {
            ...
            if(psm == bta_jv_cb.free_psm_list[i])
            {
...
void bta_jv_l2cap_write_fixed(tBTA_JV_MSG *p_data)
{
    ...
    wm_memcpy(evt_data.addr, ls->addr, sizeof(evt_data.addr));
    evt_data.req_id  = ls->req_id;
    evt_data.len     = 0;
    wm_memcpy(((uint8_t *)(msg + 1)) + L2CAP_MIN_OFFSET, ls->p_data, ls->len);
    msg->len = ls->len;
                                        ...
                                        new_st)
{
    ...
    tBTA_JV_API_PM_STATE_CHANGE *p_msg =
                    (tBTA_JV_API_PM_STATE_CHANGE *)GKI_getbuf(sizeof(tBTA_JV_API_PM_STATE_CHANGE));
...
static void fcchan_conn_chng_cbk(uint16_t chan, BD_ADDR bd_addr, uint8_t connected, uint16_t reason, tBT_TRANSPORT transport)
{
    tBTA_JV init_evt;
    tBTA_JV open_evt;
    ...
    if(tc)
    {
        ...
        else
        {
            ...
            if(t)
            {
                ...
                if(new_conn)
                {
                    wm_memcpy(&new_conn->remote_addr, bd_addr, sizeof(new_conn->remote_addr));
    ...
    if(t)
    {
        ...
        wm_memcpy(&open_evt.l2c_le_open.rem_bda, &t->remote_addr, sizeof(open_evt.l2c_le_open.rem_bda));
...
void bta_jv_l2cap_connect_le(tBTA_JV_MSG *p_data)
{
    ...
    wm_memcpy(&t->remote_addr, &cc->peer_bd_addr, sizeof(t->remote_addr));
    id = t->id;
    t->init_called = FALSE;
