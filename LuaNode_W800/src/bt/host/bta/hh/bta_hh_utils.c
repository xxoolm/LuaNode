...
void bta_hh_clean_up_kdev(tBTA_HH_DEV_CB *p_cb)
{
    ...
    GKI_free_and_reset_buf((void **)&p_cb->dscp_info.descriptor.dsc_list);
    wm_memset(p_cb, 0, sizeof(tBTA_HH_DEV_CB));     /* Reset control block */
    ...
    if(p_dscp_info)
    {
        ...
        if(p_dscp_info->dl_len)
        {
            p_cb->dscp_info.descriptor.dsc_list =
                            (uint8_t *)GKI_getbuf(p_dscp_info->dl_len);
            p_cb->dscp_info.descriptor.dl_len = p_dscp_info->dl_len;
            wm_memcpy(p_cb->dscp_info.descriptor.dsc_list, p_dscp_info->dsc_list,
                      p_dscp_info->dl_len);
        }
    }
}
...
**
*******************************************************************************/
uint8_t bta_hh_tod_spt(tBTA_HH_DEV_CB *p_cb, uint8_t sub_class)
{
    uint8_t    xx;
                            ...
                            uint16_t report_len)
{
    ...
    wm_memset(this_report, 0, BTA_HH_MAX_RPT_CHARS);
    wm_memset(p_data, 0, sizeof(tBTA_HH_KEYBD_RPT));
    wm_memcpy(this_report, p_report, report_len);
    ...
    for(xx = 0; xx < report_len; xx++)
    {
        ...
        if(this_report[xx] == BTA_HH_KB_CAPS_LOCK)
        {
            ...
            if(this_report[xx] == BTA_HH_KB_NUM_LOCK)
            {
    ...
    wm_memset(p_kb->last_report, 0, BTA_HH_MAX_RPT_CHARS);
    wm_memcpy(p_kb->last_report, p_report, report_len);
    return;
}
...
void bta_hh_cleanup_disable(tBTA_HH_STATUS status)
{
    ...
    for(xx = 0; xx < BTA_HH_MAX_DEVICE; xx ++)
    {
        GKI_free_and_reset_buf((void **)&bta_hh_cb.kdev[xx].dscp_info.descriptor.dsc_list);
    }

    GKI_free_and_reset_buf((void **)&bta_hh_cb.p_disc_db);
    (* bta_hh_cb.p_cback)(BTA_HH_DISABLE_EVT, (tBTA_HH *)&status);
    /* all connections are down, no waiting for diconnect */
    wm_memset(&bta_hh_cb, 0, sizeof(tBTA_HH_CB));
}
