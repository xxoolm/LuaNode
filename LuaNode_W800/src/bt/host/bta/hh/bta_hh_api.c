...
void BTA_HhEnable(tBTA_SEC sec_mask, tBTA_HH_CBACK *p_cback)
{
    tBTA_HH_API_ENABLE *p_buf =
                    (tBTA_HH_API_ENABLE *)GKI_getbuf(sizeof(tBTA_HH_API_ENABLE));
...
void BTA_HhOpen(BD_ADDR dev_bda, tBTA_HH_PROTO_MODE mode, tBTA_SEC sec_mask)
{
    tBTA_HH_API_CONN *p_buf =
                    (tBTA_HH_API_CONN *)GKI_getbuf(sizeof(tBTA_HH_API_CONN));
                                 ...
                                 uint16_t data, uint8_t rpt_id, BT_HDR  *p_data)
{
    tBTA_HH_CMD_DATA *p_buf =
                    (tBTA_HH_CMD_DATA *)GKI_getbuf(sizeof(tBTA_HH_CMD_DATA));
                  ...
                  uint8_t app_id, tBTA_HH_DEV_DSCP_INFO dscp_info)
{
    ...
    tBTA_HH_MAINT_DEV *p_buf = (tBTA_HH_MAINT_DEV *)GKI_getbuf(len);
...
void BTA_HhRemoveDev(uint8_t dev_handle)
{
    tBTA_HH_MAINT_DEV *p_buf =
                    (tBTA_HH_MAINT_DEV *)GKI_getbuf(sizeof(tBTA_HH_MAINT_DEV));
    p_buf->hdr.event = BTA_HH_API_MAINT_DEV_EVT;
...
void BTA_HhUpdateLeScanParam(uint8_t dev_handle, uint16_t scan_int, uint16_t scan_win)
{
    tBTA_HH_SCPP_UPDATE *p_buf =
                    (tBTA_HH_SCPP_UPDATE *)GKI_getbuf(sizeof(tBTA_HH_SCPP_UPDATE));
