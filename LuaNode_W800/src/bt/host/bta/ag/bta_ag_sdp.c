...
static void bta_ag_sdp_cback(uint16_t status, uint8_t idx)
{
    ...
    if((p_scb = bta_ag_scb_by_idx(idx)) != NULL)
    {
        ...
        tBTA_AG_DISC_RESULT *p_buf =
                        (tBTA_AG_DISC_RESULT *)GKI_getbuf(sizeof(tBTA_AG_DISC_RESULT));
...
**
******************************************************************************/
uint8_t bta_ag_add_record(uint16_t service_uuid, char *p_service_name, uint8_t scn,
                          tBTA_AG_FEAT features, uint32_t sdp_handle)
{
    ...
    uint8_t               network;
    uint8_t             result = TRUE;
    uint8_t             codec_supported = FALSE;
    uint8_t               buf[2];
    APPL_TRACE_DEBUG("bta_ag_add_record uuid: %x", service_uuid);
    wm_memset(proto_elem_list, 0, BTA_AG_NUM_PROTO_ELEMS * sizeof(tSDP_PROTOCOL_ELEM));
...
**
*******************************************************************************/
uint8_t bta_ag_sdp_find_attr(tBTA_AG_SCB *p_scb, tBTA_SERVICE_MASK service)
{
    ...
    if(service & BTA_HFP_SERVICE_MASK)
    {
        ...
        if(service & BTA_HSP_SERVICE_MASK && p_scb->role == BTA_AG_INT)
        {
...
void bta_ag_do_disc(tBTA_AG_SCB *p_scb, tBTA_SERVICE_MASK service)
{
    ...
    /* HFP acceptor; get features */
        ...
        if(service & BTA_HFP_SERVICE_MASK && p_scb->role == BTA_AG_ACP)
        {
            attr_list[0] = ATTR_ID_SERVICE_CLASS_ID_LIST;
...
{
    UNUSED(p_data);
    GKI_free_and_reset_buf((void **)&p_scb->p_disc_db);
}
#endif
