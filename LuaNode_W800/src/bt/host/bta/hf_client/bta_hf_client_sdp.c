...
static void bta_hf_client_sdp_cback(uint16_t status)
{
    ...
    tBTA_HF_CLIENT_DISC_RESULT *p_buf =
                    (tBTA_HF_CLIENT_DISC_RESULT *)GKI_getbuf(sizeof(tBTA_HF_CLIENT_DISC_RESULT));
...
**
******************************************************************************/
uint8_t bta_hf_client_add_record(char *p_service_name, uint8_t scn,
                                 tBTA_HF_CLIENT_FEAT features, uint32_t sdp_handle)
{
    ...
    wm_memset(proto_elem_list, 0, BTA_HF_CLIENT_NUM_PROTO_ELEMS * sizeof(tSDP_PROTOCOL_ELEM));
...
**
*******************************************************************************/
uint8_t bta_hf_client_sdp_find_attr(void)
{
...
void bta_hf_client_do_disc(void)
{
    ...
    bta_hf_client_cb.scb.p_disc_db = (tSDP_DISCOVERY_DB *)GKI_getbuf(BT_DEFAULT_BUFFER_SIZE);
...
{
    UNUSED(p_data);
    GKI_free_and_reset_buf((void **)&bta_hf_client_cb.scb.p_disc_db);
}
#endif
