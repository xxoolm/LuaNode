...
void BTA_GATTC_AppRegister(tBT_UUID *p_app_uuid, tBTA_GATTC_CBACK *p_client_cb)
{
    tBTA_GATTC_API_REG *p_buf =
                    (tBTA_GATTC_API_REG *)GKI_getbuf(sizeof(tBTA_GATTC_API_REG));
...
void BTA_GATTC_AppDeregister(tBTA_GATTC_IF client_if)
{
    tBTA_GATTC_API_DEREG *p_buf =
                    (tBTA_GATTC_API_DEREG *)GKI_getbuf(sizeof(tBTA_GATTC_API_DEREG));
...
void BTA_GATTC_Open(tBTA_GATTC_IF client_if, BD_ADDR remote_bda,
                    uint8_t is_direct, tBTA_GATT_TRANSPORT transport)
{
    tBTA_GATTC_API_OPEN *p_buf =
                    (tBTA_GATTC_API_OPEN *) GKI_getbuf(sizeof(tBTA_GATTC_API_OPEN));
...
**
*******************************************************************************/
void BTA_GATTC_CancelOpen(tBTA_GATTC_IF client_if, BD_ADDR remote_bda, uint8_t is_direct)
{
    tBTA_GATTC_API_CANCEL_OPEN *p_buf =
                    (tBTA_GATTC_API_CANCEL_OPEN *)GKI_getbuf(sizeof(tBTA_GATTC_API_CANCEL_OPEN));
...
void BTA_GATTC_ConfigureMTU(uint16_t conn_id, uint16_t mtu)
{
    tBTA_GATTC_API_CFG_MTU *p_buf =
                    (tBTA_GATTC_API_CFG_MTU *)GKI_getbuf(sizeof(tBTA_GATTC_API_CFG_MTU));
...
void BTA_GATTC_ServiceSearchRequest(uint16_t conn_id, tBT_UUID *p_srvc_uuid)
{
    ...
    tBTA_GATTC_API_SEARCH *p_buf = (tBTA_GATTC_API_SEARCH *)GKI_getbuf(len);
    ...
    if(p_srvc_uuid)
    {
        ...
        wm_memcpy(p_buf->p_srvc_uuid, p_srvc_uuid, sizeof(tBT_UUID));
    }
    else
    {
...
**                  count: number of elements in database.note: -1 invalid;
**
*******************************************************************************/
...
void BTA_GATTC_ReadCharacteristic(uint16_t conn_id, uint16_t handle, tBTA_GATT_AUTH_REQ auth_req)
{
    tBTA_GATTC_API_READ *p_buf =
                    (tBTA_GATTC_API_READ *)GKI_getbuf(sizeof(tBTA_GATTC_API_READ));
                            ...
                            tBTA_GATT_AUTH_REQ auth_req)
{
    tBTA_GATTC_API_READ_MULTI *p_buf =
                    (tBTA_GATTC_API_READ_MULTI *)GKI_getbuf(sizeof(tBTA_GATTC_API_READ_MULTI));
    ...
    if(p_buf->num_attr > 0)
    {
        wm_memcpy(p_buf->handles, p_read_multi->handles, sizeof(uint16_t) * p_read_multi->num_attr);
    }
                              ...
                              tBTA_GATT_AUTH_REQ auth_req)
{
    tBTA_GATTC_API_WRITE  *p_buf =
                    (tBTA_GATTC_API_WRITE *)GKI_getbuf(sizeof(tBTA_GATTC_API_WRITE) + len);
    ...
    if(p_value && len > 0)
    {
        ...
        wm_memcpy(p_buf->p_value, p_value, len);
    }
                              ...
                              tBTA_GATT_AUTH_REQ auth_req)
{
    ...
    tBTA_GATTC_API_WRITE *p_buf = (tBTA_GATTC_API_WRITE *)GKI_getbuf(len);
    p_buf->hdr.event = BTA_GATTC_API_WRITE_EVT;
    p_buf->hdr.layer_specific = conn_id;
    p_buf->auth_req = auth_req;
    p_buf->handle = handle;
    ...
    if(p_data && p_data->len != 0)
    {
        ...
        /* pack the descr data */
        wm_memcpy(p_buf->p_value, p_data->p_value, p_data->len);
    }
...
void BTA_GATTC_PrepareWrite(uint16_t conn_id, uint16_t handle,
                            uint16_t offset, uint16_t len, uint8_t *p_value,
                            tBTA_GATT_AUTH_REQ auth_req)
{
    tBTA_GATTC_API_WRITE *p_buf =
                    (tBTA_GATTC_API_WRITE *)GKI_getbuf(sizeof(tBTA_GATTC_API_WRITE) + len);
...
**
*******************************************************************************/
void BTA_GATTC_ExecuteWrite(uint16_t conn_id, uint8_t is_execute)
{
    tBTA_GATTC_API_EXEC *p_buf =
                    (tBTA_GATTC_API_EXEC *)GKI_getbuf(sizeof(tBTA_GATTC_API_EXEC));
...
void BTA_GATTC_SendIndConfirm(uint16_t conn_id, uint16_t handle)
{
    tBTA_GATTC_API_CONFIRM *p_buf =
                    (tBTA_GATTC_API_CONFIRM *)GKI_getbuf(sizeof(tBTA_GATTC_API_CONFIRM));
    ...
    if((p_clreg = bta_gattc_cl_get_regcb(client_if)) != NULL)
    {
        ...
        if(status != BTA_GATT_OK)
        {
            for(i = 0; i < BTA_GATTC_NOTIF_REG_MAX; i ++)
            {
                if(!p_clreg->notif_reg[i].in_use)
                {
                    wm_memset((void *)&p_clreg->notif_reg[i], 0, sizeof(tBTA_GATTC_NOTIF_REG));
                    p_clreg->notif_reg[i].in_use = TRUE;
                    wm_memcpy(p_clreg->notif_reg[i].remote_bda, bda, BD_ADDR_LEN);
                    p_clreg->notif_reg[i].handle = handle;
                    status = BTA_GATT_OK;
                    break;
                }
        ...
        BD_ADDR bda, uint16_t handle)
{
    ...
    for(int i = 0; i < BTA_GATTC_NOTIF_REG_MAX; i ++)
    {
                ...
                p_clreg->notif_reg[i].handle == handle)
        {
            ...
            wm_memset(&p_clreg->notif_reg[i], 0, sizeof(tBTA_GATTC_NOTIF_REG));
            return BTA_GATT_OK;
        }
    }
...
**
*******************************************************************************/
void BTA_GATTC_Listen(tBTA_GATTC_IF client_if, uint8_t start, BD_ADDR_PTR target_bda)
{
    tBTA_GATTC_API_LISTEN *p_buf =
                    (tBTA_GATTC_API_LISTEN *)GKI_getbuf(sizeof(tBTA_GATTC_API_LISTEN) + BD_ADDR_LEN);
...
**
*******************************************************************************/
void BTA_GATTC_Broadcast(tBTA_GATTC_IF client_if, uint8_t start)
{
    tBTA_GATTC_API_LISTEN *p_buf =
                    (tBTA_GATTC_API_LISTEN *)GKI_getbuf(sizeof(tBTA_GATTC_API_LISTEN) + BD_ADDR_LEN);
