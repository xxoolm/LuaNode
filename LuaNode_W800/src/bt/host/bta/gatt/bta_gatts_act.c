...
static void bta_gatts_nv_save_cback(uint8_t is_saved, tGATTS_HNDL_RANGE *p_hndl_range);
static uint8_t bta_gatts_nv_srv_chg_cback(tGATTS_SRV_CHG_CMD cmd, tGATTS_SRV_CHG_REQ *p_req,
...
static void bta_gatts_cong_cback(uint16_t conn_id, uint8_t congested);
...
**
*******************************************************************************/
static void bta_gatts_nv_save_cback(uint8_t is_add, tGATTS_HNDL_RANGE *p_hndl_range)
{
...
**
*******************************************************************************/
static uint8_t bta_gatts_nv_srv_chg_cback(tGATTS_SRV_CHG_CMD cmd,
    ...
    if(p_cb->enabled)
    {
        for(i = 0; i < BTA_GATTS_MAX_APP_NUM; i ++)
        {
            if(p_cb->rcb[i].in_use)
            {
				...
				/** Fixed dereg application, automatically*/
				if(p_cback)
			    {
...
void bta_gatts_register(tBTA_GATTS_CB *p_cb, tBTA_GATTS_DATA *p_msg)
{
    ...
    if(status == BTA_GATT_OK)
    {
        ...
        wm_memcpy(&cb_data.reg_oper.uuid, &p_msg->api_reg.app_uuid, sizeof(tBT_UUID));
        ...
        if(first_unuse != 0xff)
        {
            ...
            wm_memcpy(&p_cb->rcb[first_unuse].app_uuid, &p_msg->api_reg.app_uuid, sizeof(tBT_UUID));
            ...
            else
            {
                tBTA_GATTS_INT_START_IF *p_buf =
                                (tBTA_GATTS_INT_START_IF *)GKI_getbuf(sizeof(tBTA_GATTS_INT_START_IF));
...
void bta_gatts_deregister(tBTA_GATTS_CB *p_cb, tBTA_GATTS_DATA *p_msg)
{
    ...
    for(i = 0; i < BTA_GATTS_MAX_APP_NUM; i ++)
    {
        if(p_cb->rcb[i].in_use && p_cb->rcb[i].gatt_if == p_msg->api_dereg.server_if)
        {
            ...
            GATT_Deregister(p_cb->rcb[i].gatt_if);
            /* reset cb */
            wm_memset(&p_cb->rcb[i], 0, sizeof(tBTA_GATTS_RCB));
            cb_data.reg_oper.status = status;

			/*Try to reset srvc cb£¬worked when deregisting, free the srvc db, bug ID 1532*/
			bta_gatts_free_srvc_cb(p_cb, i);
            break;
        }
    }
...
void bta_gatts_create_srvc(tBTA_GATTS_CB *p_cb, tBTA_GATTS_DATA *p_msg)
{
    ...
    if(rcb_idx != BTA_GATTS_INVALID_APP)
    {
        if((srvc_idx = bta_gatts_alloc_srvc_cb(p_cb, rcb_idx)) != BTA_GATTS_INVALID_APP)
        {
            ...
            if(service_id != 0)
            {
                wm_memcpy(&p_cb->srvc_cb[srvc_idx].service_uuid,
            ...
            else
            {
                ...
                wm_memset(&p_cb->srvc_cb[srvc_idx], 0, sizeof(tBTA_GATTS_SRVC_CB));
                APPL_TRACE_ERROR("service creation failed.");
            }

            wm_memcpy(&cb_data.create.uuid, &p_msg->api_create_svc.service_uuid, sizeof(tBT_UUID));
...
void bta_gatts_add_char(tBTA_GATTS_SRVC_CB *p_srvc_cb, tBTA_GATTS_DATA *p_msg)
{
    ...
    cb_data.add_result.service_id = p_msg->api_add_incl_srvc.hdr.layer_specific;
    cb_data.add_result.attr_id = attr_id;
    wm_memcpy(&cb_data.add_result.char_uuid, &p_msg->api_add_char.char_uuid, sizeof(tBT_UUID));
...
void bta_gatts_add_char_descr(tBTA_GATTS_SRVC_CB *p_srvc_cb, tBTA_GATTS_DATA *p_msg)
{
    ...
    cb_data.add_result.service_id = p_msg->api_add_incl_srvc.hdr.layer_specific;
    cb_data.add_result.attr_id = attr_id;
    wm_memcpy(&cb_data.add_result.char_uuid, &p_msg->api_add_char_descr.descr_uuid, sizeof(tBT_UUID));
...
void bta_gatts_delete_service(tBTA_GATTS_SRVC_CB *p_srvc_cb, tBTA_GATTS_DATA *p_msg)
{
                           ...
                           p_srvc_cb->inst_num))
    {
        ...
        wm_memset(p_srvc_cb, 0, sizeof(tBTA_GATTS_SRVC_CB));
    }
    else
    {
...
void bta_gatts_stop_service(tBTA_GATTS_SRVC_CB *p_srvc_cb, tBTA_GATTS_DATA *p_msg)
{
    ...
    cb_data.srvc_oper.status = BTA_GATT_OK;
    APPL_TRACE_DEBUG("bta_gatts_stop_service service_id= %d\r\n", p_srvc_cb->service_id);
        ...
        tGATTS_REQ_TYPE req_type, tGATTS_DATA *p_data)
{
    ...
    tGATT_IF            gatt_if;
    tBTA_GATT_TRANSPORT transport;
    wm_memset(&cb_data, 0, sizeof(tBTA_GATTS));
                                 ...
                                 tGATT_TRANSPORT transport)
{
    ...
    if(p_reg && p_reg->p_cback)
    {
        ...
        wm_memcpy(cb_data.conn.remote_bda, bda, BD_ADDR_LEN);
