...
**
*******************************************************************************/
void rfc_send_dm(tRFC_MCB *p_mcb, uint8_t dlci, uint8_t pf)
{
    uint8_t   *p_data;
    uint8_t   cr = RFCOMM_CR(p_mcb->is_initiator, FALSE);
    BT_HDR  *p_buf = (BT_HDR *)GKI_getbuf(RFCOMM_CMD_BUF_SIZE);
...
**
*******************************************************************************/
void rfc_send_pn(tRFC_MCB *p_mcb, uint8_t dlci, uint8_t is_command, uint16_t mtu, uint8_t cl, uint8_t k)
{
    uint8_t    *p_data;
    BT_HDR   *p_buf = (BT_HDR *)GKI_getbuf(RFCOMM_CMD_BUF_SIZE);
...
**
*******************************************************************************/
void rfc_send_fcon(tRFC_MCB *p_mcb, uint8_t is_command)
{
    uint8_t   *p_data;
    BT_HDR  *p_buf = (BT_HDR *)GKI_getbuf(RFCOMM_CMD_BUF_SIZE);
    p_buf->offset = L2CAP_MIN_OFFSET + RFCOMM_CTRL_FRAME_LEN;
...
**
*******************************************************************************/
void rfc_send_fcoff(tRFC_MCB *p_mcb, uint8_t is_command)
{
    uint8_t   *p_data;
    BT_HDR  *p_buf = (BT_HDR *)GKI_getbuf(RFCOMM_CMD_BUF_SIZE);
    p_buf->offset = L2CAP_MIN_OFFSET + RFCOMM_CTRL_FRAME_LEN;
...
**
*******************************************************************************/
void rfc_send_msc(tRFC_MCB *p_mcb, uint8_t dlci, uint8_t is_command,
                  tPORT_CTRL *p_pars)
{
    uint8_t   *p_data;
    uint8_t   signals;
...
**
*******************************************************************************/
void rfc_send_rls(tRFC_MCB *p_mcb, uint8_t dlci, uint8_t is_command, uint8_t status)
{
    uint8_t   *p_data;
    BT_HDR  *p_buf = (BT_HDR *)GKI_getbuf(RFCOMM_CMD_BUF_SIZE);
    p_buf->offset = L2CAP_MIN_OFFSET + RFCOMM_CTRL_FRAME_LEN;
...
**
*******************************************************************************/
void rfc_send_rpn(tRFC_MCB *p_mcb, uint8_t dlci, uint8_t is_command,
...
**
*******************************************************************************/
void rfc_send_test(tRFC_MCB *p_mcb, uint8_t is_command, BT_HDR *p_buf)
{
    ...
    if(p_buf->offset < (L2CAP_MIN_OFFSET + RFCOMM_MIN_OFFSET + 2))
    {
        ...
        BT_HDR *p_new_buf = (BT_HDR *) GKI_getbuf(p_buf->len + (L2CAP_MIN_OFFSET +
    ...
    switch(p_frame->type)
    {
        ...
        case RFCOMM_UIH:
            if(!RFCOMM_VALID_DLCI(p_frame->dlci))
            {
                ...
                if(!rfc_check_fcs(2, p_start, fcs))
                {
