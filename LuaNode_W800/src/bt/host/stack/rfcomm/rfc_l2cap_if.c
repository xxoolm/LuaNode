...
static void RFCOMM_DisconnectInd(uint16_t lcid, uint8_t is_clear);
static void RFCOMM_QoSViolationInd(BD_ADDR bd_addr);
static void RFCOMM_BufDataInd(uint16_t lcid, BT_HDR *p_buf);
static void RFCOMM_CongestionStatusInd(uint16_t lcid, uint8_t is_congested);
...
void RFCOMM_ConnectInd(BD_ADDR bd_addr, uint16_t lcid, uint16_t psm, uint8_t id)
{
    ...
    if((p_mcb) && (p_mcb->state != RFC_MX_STATE_IDLE))
    {
        ...
        if((p_mcb->is_initiator) && (p_mcb->state == RFC_MX_STATE_WAIT_CONN_CNF))
        {
            ...
            rfc_timer_start(p_mcb, (uint16_t)(GKI_time_get_os_boottime_ms() % 10 + 2));
            return;
        }
        else
        {
...
**
*******************************************************************************/
void RFCOMM_DisconnectInd(uint16_t lcid, uint8_t is_conf_needed)
{
    tRFC_MCB *p_mcb = rfc_find_lcid_mcb(lcid);
...
**
*******************************************************************************/
void RFCOMM_CongestionStatusInd(uint16_t lcid, uint8_t is_congested)
{
    tRFC_MCB *p_mcb = rfc_find_lcid_mcb(lcid);
