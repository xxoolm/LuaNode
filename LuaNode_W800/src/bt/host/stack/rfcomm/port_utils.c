...
void port_set_defaults(tPORT *p_port)
{
    ...
    p_port->credit_tx      = 0;
    p_port->credit_rx      = 0;
    wm_memset(&p_port->local_ctrl, 0, sizeof(p_port->local_ctrl));
    wm_memset(&p_port->peer_ctrl, 0, sizeof(p_port->peer_ctrl));
    wm_memset(&p_port->rx, 0, sizeof(p_port->rx));
    wm_memset(&p_port->tx, 0, sizeof(p_port->tx));
