...
/********************************************************************************/
static void gatt_le_connect_cback(uint16_t chan, BD_ADDR bd_addr, uint8_t connected,
                                  uint16_t reason, tBT_TRANSPORT transport);
static void gatt_le_data_ind(uint16_t chan, BD_ADDR bd_addr, BT_HDR *p_buf);
static void gatt_le_cong_cback(BD_ADDR remote_bda, uint8_t congest);
...
static void gatt_l2cif_disconnect_ind_cback(uint16_t l2cap_cid, uint8_t ack_needed);
...
static void gatt_l2cif_congest_cback(uint16_t cid, uint8_t congested);

#define GATT_OPCODE_MSG_DEBUG 0

#if GATT_OPCODE_MSG_DEBUG == 1
    static char *gatt_opcode_2_str(uint8_t opcode);
#endif
...
void gatt_init(void)
{
	...
	gatt_cb_ptr = (tGATT_CB *)GKI_os_malloc(sizeof(tGATT_CB));
	#endif
    wm_memset(&gatt_cb, 0, sizeof(tGATT_CB));
    wm_memset(&fixed_reg, 0, sizeof(tL2CAP_FIXED_CHNL_REG));
...
void gatt_free(void)
{
	...
	#if GATT_DYNAMIC_MEMORY == TRUE
	if(gatt_cb_ptr) GKI_os_free(gatt_cb_ptr);
	#endif
}
...
**
*******************************************************************************/
uint8_t gatt_connect(BD_ADDR rem_bda, tGATT_TCB *p_tcb, tBT_TRANSPORT transport)
{
    uint8_t             gatt_ret = FALSE;
...
**
*******************************************************************************/
uint8_t gatt_disconnect(tGATT_TCB *p_tcb)
{
    uint8_t             ret = FALSE;
...
**
*******************************************************************************/
void gatt_update_app_hold_link_status(tGATT_IF gatt_if, tGATT_TCB *p_tcb, uint8_t is_add)
{
    uint8_t i;
    uint8_t found = FALSE;
...
**
*******************************************************************************/
void gatt_update_app_use_link_flag(tGATT_IF gatt_if, tGATT_TCB *p_tcb, uint8_t is_add, uint8_t check_acl_link)
{
...
**
*******************************************************************************/
uint8_t gatt_act_connect(tGATT_REG *p_reg, BD_ADDR bd_addr, tBT_TRANSPORT transport)
{
    uint8_t     ret = FALSE;
    tGATT_TCB   *p_tcb;
    uint8_t       st;
...
**
*******************************************************************************/
static void gatt_channel_congestion(tGATT_TCB *p_tcb, uint8_t congested)
{
    uint8_t i = 0;
...
**
*******************************************************************************/
static void gatt_le_cong_cback(BD_ADDR remote_bda, uint8_t congested)
{
...
**
*******************************************************************************/
void gatt_l2cif_disconnect_ind_cback(uint16_t lcid, uint8_t ack_needed)
{
    tGATT_TCB       *p_tcb;
    uint16_t          reason;
...
**
*******************************************************************************/
static void gatt_l2cif_congest_cback(uint16_t lcid, uint8_t congested)
{
...
void gatt_add_a_bonded_dev_for_srv_chg(BD_ADDR bda)
{
    ...
    wm_memcpy(srv_chg_clt.bda, bda, BD_ADDR_LEN);
    srv_chg_clt.srv_changed = FALSE;

    if(gatt_add_srv_chg_clt(&srv_chg_clt) != NULL)
    {
        wm_memcpy(req.srv_chg.bda, bda, BD_ADDR_LEN);
    ...
    if(gatt_cb.cb_info.p_srv_chg_callback)
    {
        ...
        if(status && rsp.num_clients)
        {
            ...
            while((i <= num_clients) && status)
            {
                ...
                {
                    wm_memcpy(&srv_chg_clt, &rsp.srv_chg, sizeof(tGATTS_SRV_CHG));
...
static char *gatt_opcode_2_str(uint8_t opcode)
{
    switch(opcode)
    {
            ...
            CASE_RETURN_STR(GATT_REQ_EXEC_WRITE)
            CASE_RETURN_STR(GATT_RSP_EXEC_WRITE)
            CASE_RETURN_STR(GATT_HANDLE_VALUE_NOTIF)
            CASE_RETURN_STR(GATT_HANDLE_VALUE_IND)
            CASE_RETURN_STR(GATT_HANDLE_VALUE_CONF)
            CASE_RETURN_STR(GATT_SIGN_CMD_WRITE)

        default:
            return "Unknown gatt opcode\r\n";
    }
}
#endif
#endif /* BLE_INCLUDED */
