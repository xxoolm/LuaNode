...
static uint8_t gatts_db_add_service_declaration(tGATT_SVC_DB *p_db, tBT_UUID *p_service, uint8_t is_pri);
        ...
        if(uuid16 == GATT_UUID_CHAR_DECLARE)
        {
            ...
            if(mtu >= len)
            {
                ...
                /* convert a 32bits UUID to 128 bits */
                    ...
                    if(((tGATT_ATTR32 *)(p_attr16->p_next))->uuid_type == GATT_ATTR_UUID_TYPE_32)
                    {
    ...
    if(p_db && p_db->p_attr_list)
    {
        ...
        while(p_attr && p_attr->handle <= e_handle)
        {
            if(p_attr->uuid_type == GATT_ATTR_UUID_TYPE_16)
            {
                ...
                if(p_attr->uuid_type == GATT_ATTR_UUID_TYPE_32)
                {
            ...
            if(p_attr->handle == handle)
            {
                        ...
                        && (perm & GATT_WRITE_SIGNED_PERM))
                {
                    ...
                    /* LE security mode 2 level 2 and security mode 1 and LE */
                        ...
                        if(((perm & GATT_PERM_WRITE_SIGNED_MITM) && (perm & GATT_PERM_WRITE_ENCRYPTED)) ||
                    ...
                    /* require authentication, but not been authenticated */
                        ...
                        if((perm & GATT_WRITE_AUTH_REQUIRED) && !(sec_flag & GATT_SEC_FLAG_LKEY_UNAUTHED))
                        {
                            ...
                            if((perm & GATT_WRITE_MITM_REQUIRED) && !(sec_flag & GATT_SEC_FLAG_LKEY_AUTHED))
                            {
                                ...
                                if((perm & GATT_WRITE_ENCRYPTED_PERM) && !(sec_flag & GATT_SEC_FLAG_ENCRYPTED))
                                {
                                    status = GATT_INSUF_ENCRYPTION;
                                    GATT_TRACE_ERROR("gatts_write_attr_perm_check - GATT_INSUF_ENCRYPTION");
                                }
                                else
                                    if((perm & GATT_WRITE_ENCRYPTED_PERM) && (sec_flag & GATT_SEC_FLAG_ENCRYPTED) && (key_size < min_key_size))
                                    {
                                    ...
                                    /* LE security mode 2 attribute  */
                                        ...
                                        if(perm & GATT_WRITE_SIGNED_PERM && op_code != GATT_SIGN_CMD_WRITE && !(sec_flag & GATT_SEC_FLAG_ENCRYPTED)
                                        ...
                                        else /* writable: must be char value declaration or char descritpors */
                                        {
                                            if(p_attr->uuid_type == GATT_ATTR_UUID_TYPE_16)
                                            {
                                                ...
                                                if(p_attr->uuid_type == GATT_ATTR_UUID_TYPE_128 ||
                                            ...
                                            /* these attribute does not allow write blob */
                                                         ...
                                                         p_attr->uuid == GATT_UUID_CHAR_SRVR_CONFIG))
                                                {
                                                    if(op_code == GATT_REQ_PREPARE_WRITE && offset != 0)   /* does not allow write blob */
                                                    {
                                                        ...
                                                        if(len != max_size)     /* data does not match the required format */
                                                        {
...
static void *allocate_attr_in_db(tGATT_SVC_DB *p_db, tBT_UUID *p_uuid, tGATT_PERM perm)
{
    ...
    if(p_attr16->uuid_type == GATT_ATTR_UUID_TYPE_16)
    {
        ...
        if(p_attr16->uuid_type == GATT_ATTR_UUID_TYPE_32)
        {
