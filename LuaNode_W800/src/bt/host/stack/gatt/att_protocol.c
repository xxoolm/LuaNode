...
BT_HDR *attp_build_mtu_cmd(uint8_t op_code, uint16_t rx_mtu)
{
    uint8_t *p;
    BT_HDR *p_buf =
                    (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + GATT_HDR_SIZE + L2CAP_MIN_OFFSET);
...
{
    uint8_t *p;
    BT_HDR *p_buf = (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + L2CAP_MIN_OFFSET + 5);
    p = (uint8_t *)(p_buf + 1) + L2CAP_MIN_OFFSET;
...
BT_HDR *attp_build_browse_cmd(uint8_t op_code, uint16_t s_hdl, uint16_t e_hdl, tBT_UUID uuid)
{
    ...
    BT_HDR *p_buf = (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + payload_size + L2CAP_MIN_OFFSET);
...
BT_HDR *attp_build_read_by_type_value_cmd(uint16_t payload_size, tGATT_FIND_TYPE_VALUE *p_value_type)
{
    ...
    BT_HDR *p_buf =
                    (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + payload_size + L2CAP_MIN_OFFSET);
    p = (uint8_t *)(p_buf + 1) + L2CAP_MIN_OFFSET;
    p_buf->offset = L2CAP_MIN_OFFSET;
    ...
    wm_memcpy(p, p_value_type->value, len);
    p_buf->len += len;
    return p_buf;
}
...
BT_HDR *attp_build_read_multi_cmd(uint16_t payload_size, uint16_t num_handle, uint16_t *p_handle)
{
    uint8_t *p, i = 0;
    BT_HDR *p_buf =
                    (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + num_handle * 2 + 1 + L2CAP_MIN_OFFSET);
    p = (uint8_t *)(p_buf + 1) + L2CAP_MIN_OFFSET;
    p_buf->offset = L2CAP_MIN_OFFSET;
    p_buf->len = 1;
...
BT_HDR *attp_build_handle_cmd(uint8_t op_code, uint16_t handle, uint16_t offset)
{
    uint8_t *p;
    BT_HDR *p_buf =
                    (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + 5 + L2CAP_MIN_OFFSET);
    p = (uint8_t *)(p_buf + 1) + L2CAP_MIN_OFFSET;
    p_buf->offset = L2CAP_MIN_OFFSET;
    UINT8_TO_STREAM(p, op_code);
    p_buf->len  = 1;
...
BT_HDR *attp_build_opcode_cmd(uint8_t op_code)
{
    uint8_t *p;
    BT_HDR *p_buf =
                    (BT_HDR *)GKI_getbuf(sizeof(BT_HDR) + 1 + L2CAP_MIN_OFFSET);
    p = (uint8_t *)(p_buf + 1) + L2CAP_MIN_OFFSET;
    p_buf->offset = L2CAP_MIN_OFFSET;
    UINT8_TO_STREAM(p, op_code);
    p_buf->len  = 1;
...
tGATT_STATUS attp_send_msg_to_l2cap(tGATT_TCB *p_tcb, BT_HDR *p_toL2CAP)
{
    ...
    if(l2cap_ret == L2CAP_DW_FAILED)
    {
        ...
        if(l2cap_ret == L2CAP_DW_CONGESTED)
        {
