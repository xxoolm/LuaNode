...
void gatt_act_read(tGATT_CLCB *p_clcb, uint16_t offset)
{
    ...
    switch(p_clcb->op_subtype)
    {
        ...
        case GATT_READ_MULTIPLE:
            ...
            wm_memcpy(&msg.read_multi, p_clcb->p_attr_buf, sizeof(tGATT_READ_MULTI));
            break;
...
void gatt_process_find_type_value_rsp(tGATT_TCB *p_tcb, tGATT_CLCB *p_clcb, uint16_t len, uint8_t *p_data)
{
    ...
    wm_memset(&result, 0, sizeof(tGATT_DISC_RES));
    result.type.len = 2;
                               ...
                               uint16_t len, uint8_t *p_data)
{
    ...
    wm_memset(&value, 0, sizeof(value));
    STREAM_TO_UINT16(value.handle, p);
    value.len = len - 2;
    wm_memcpy(value.value, p, value.len);
                                   ...
                                   uint16_t len, uint8_t *p_data)
{
    ...
    while(len >= (handle_len + value_len))
    {
        ...
        wm_memset(&result, 0, sizeof(tGATT_DISC_RES));
        wm_memset(&record_value, 0, sizeof(tGATT_DISC_VALUE));
    ...
    if(p_clcb->operation == GATTC_OPTYPE_READ)
    {
        ...
        else
        {
            ...
            if(offset < GATT_MAX_ATTR_LEN)
            {
                ...
                p_clcb->counter += len;
                wm_memcpy(p_clcb->p_attr_buf + offset, p, len);
...
**
*******************************************************************************/
uint8_t gatt_cl_send_next_cmd_inq(tGATT_TCB *p_tcb)
{
