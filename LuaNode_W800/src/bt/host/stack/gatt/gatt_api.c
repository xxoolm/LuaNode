...
uint8_t GATTS_AddHandleRange(tGATTS_HNDL_RANGE *p_hndl_range)
{
...
**
*******************************************************************************/
uint8_t  GATTS_NVRegister(tGATT_APPL_INFO *p_cb_info)
{
    uint8_t status = FALSE;
...
**
*******************************************************************************/
uint16_t GATTS_CreateService(tGATT_IF gatt_if, tBT_UUID *p_svc_uuid,
                             uint16_t svc_inst, uint16_t num_handles, uint8_t is_pri)
{
    tGATT_HDL_LIST_INFO     *p_list_info = &gatt_cb.hdl_list_info;
    tGATT_HDL_LIST_ELEM     *p_list = NULL;
    uint16_t                  s_hdl = 0;
    uint8_t                 save_hdl = FALSE;
...
**
*******************************************************************************/
uint16_t GATTS_AddIncludeService(uint16_t service_handle, uint16_t include_svc_handle)
...
**
*******************************************************************************/
uint16_t GATTS_AddCharacteristic(uint16_t service_handle, tBT_UUID *p_char_uuid,
...
**
*******************************************************************************/
uint16_t GATTS_AddCharDescriptor(uint16_t service_handle,
                                 tGATT_PERM perm,
...
**
*******************************************************************************/
uint8_t GATTS_DeleteService(tGATT_IF gatt_if, tBT_UUID *p_svc_uuid, uint16_t svc_inst)
{
    tGATT_HDL_LIST_INFO             *p_list_info = &gatt_cb.hdl_list_info;
    tGATT_HDL_LIST_ELEM             *p_list = NULL;
    uint8_t                           i_sreg;
    ...
    GATT_TRACE_DEBUG("p_list(0x%08x)released handles s_hdl=%u e_hdl=%u",p_list,
...
**
*******************************************************************************/
tGATT_STATUS GATTS_StartService(tGATT_IF gatt_if, uint16_t service_handle,
    ...
    switch(sup_transport)
    {
        ...
        case GATT_TRANSPORT_LE_BR_EDR:
            if(p_sreg->type == GATT_UUID_PRI_SERVICE)
            {
                ...
                GATT_TRACE_DEBUG("@@@!!! Add sdp record 0x%04x\r\n", p_uuid->uu.uuid16);
...
void GATTS_StopService(uint16_t service_handle)
{
    ...
    if((ii > 0) && (ii < GATT_MAX_SR_PROFILES) && (gatt_cb.sr_reg[ii].in_use))
    {
        ...
        wm_memset(&gatt_cb.sr_reg[ii], 0, sizeof(tGATT_SR_REG));
    }
    else
    {
...
tGATT_STATUS GATTS_HandleValueIndication(uint16_t conn_id,  uint16_t attr_handle, uint16_t val_len, uint8_t *p_val)
{
    ...
    wm_memcpy(indication.value, p_val, val_len);
        ...
        uint16_t val_len, uint8_t *p_val)
{
    ...
    if(GATT_HANDLE_IS_VALID(attr_handle))
    {
        ...
        notif.len       = val_len;
        wm_memcpy(notif.value, p_val, val_len);
    ...
    if((p_clcb = gatt_clcb_alloc(conn_id)) != NULL)
    {
        ...
        switch(type)
        {
            ...
            case GATT_READ_CHAR_VALUE:
                ...
                wm_memcpy(&p_clcb->uuid, &p_read->service.uuid, sizeof(tBT_UUID));
                break;
            ...
            case GATT_READ_MULTIPLE:
                ...
                tGATT_READ_MULTI *p_read_multi =
                                (tGATT_READ_MULTI *)GKI_getbuf(sizeof(tGATT_READ_MULTI));
                p_clcb->p_attr_buf = (uint8_t *)p_read_multi;
                wm_memcpy(p_read_multi, &p_read->read_multiple, sizeof(tGATT_READ_MULTI));

            case GATT_READ_BY_HANDLE:
            case GATT_READ_PARTIAL:
                wm_memset(&p_clcb->uuid, 0, sizeof(tBT_UUID));
...
tGATT_STATUS GATTC_Write(uint16_t conn_id, tGATT_WRITE_TYPE type, tGATT_VALUE *p_write)
{
    ...
    if((p_clcb = gatt_clcb_alloc(conn_id)) != NULL)
    {
        ...
        p_clcb->p_attr_buf = (uint8_t *)GKI_getbuf(sizeof(tGATT_VALUE));
        wm_memcpy(p_clcb->p_attr_buf, (void *)p_write, sizeof(tGATT_VALUE));
...
**
*******************************************************************************/
tGATT_STATUS GATTC_ExecuteWrite(uint16_t conn_id, uint8_t is_execute)
{
    tGATT_STATUS status = GATT_SUCCESS;
    tGATT_CLCB      *p_clcb;
...
void GATT_SetIdleTimeout(BD_ADDR bd_addr, uint16_t idle_tout, tBT_TRANSPORT transport)
{
    ...
    {
    	GATT_TRACE_API("p_tcb->att_lcid=%d\r\n", p_tcb->att_lcid);
...
tGATT_IF GATT_Register(tBT_UUID *p_app_uuid128, tGATT_CBACK *p_cb_info)
{
    ...
    for(i_gatt_if = 0, p_reg = gatt_cb.cl_rcb; i_gatt_if < GATT_MAX_APPS; i_gatt_if++, p_reg++)
    {
        if(!p_reg->in_use)
        {
            wm_memset(p_reg, 0, sizeof(tGATT_REG));
            i_gatt_if++;              /* one based number */
            p_reg->app_uuid128 =  *p_app_uuid128;
            gatt_if            = (tGATT_IF)i_gatt_if;
...
void GATT_Deregister(tGATT_IF gatt_if)
{
	...
	/**Try to free the p_list allocated when creating service. some lazzy man doesnot delete service before deregister the app*/
	GATT_TRACE_API("GATT_Deregister gatt_if=%d, try to free servcie db buffer", gatt_if);
    gatt_dbg_display_uuid(p_reg->app_uuid128);
	while(1)
	{
		p_list = gatt_find_hdl_buffer_by_app_id_only(&p_reg->app_uuid128);
		if(p_list)
		{
			GATT_TRACE_API("GATT_Deregister gatt_if=%d, now start to free only created service buffer", gatt_if);
			gatt_dbg_display_uuid(p_list->asgn_range.svc_uuid);
	    	GATTS_DeleteService(gatt_if,&p_list->asgn_range.svc_uuid,p_list->asgn_range.svc_inst);
		}else
		{
			GATT_TRACE_API("GATT_Deregister, all service deleted\r\n");
			break;
		}
	}
	/**************************************************************************/
...
**
*******************************************************************************/
uint8_t GATT_Connect(tGATT_IF gatt_if, BD_ADDR bd_addr, uint8_t is_direct, tBT_TRANSPORT transport)
{
    tGATT_REG    *p_reg;
    uint8_t status = FALSE;
...
**
*******************************************************************************/
uint8_t GATT_CancelConnect(tGATT_IF gatt_if, BD_ADDR bd_addr, uint8_t is_direct)
{
    tGATT_REG     *p_reg;
    tGATT_TCB     *p_tcb;
    uint8_t       status = TRUE;
...
**
*******************************************************************************/
uint8_t GATT_GetConnectionInfor(uint16_t conn_id, tGATT_IF *p_gatt_if, BD_ADDR bd_addr,
                                tBT_TRANSPORT *p_transport)
{
    ...
    if(p_tcb && p_reg)
    {
        wm_memcpy(bd_addr, p_tcb->peer_bda, BD_ADDR_LEN);
        *p_gatt_if = gatt_if;
...
**
*******************************************************************************/
uint8_t GATT_GetConnIdIfConnected(tGATT_IF gatt_if, BD_ADDR bd_addr, uint16_t *p_conn_id,
...
**
*******************************************************************************/
uint8_t GATT_Listen(tGATT_IF gatt_if, uint8_t start, BD_ADDR_PTR bd_addr)
{
    tGATT_REG    *p_reg;
