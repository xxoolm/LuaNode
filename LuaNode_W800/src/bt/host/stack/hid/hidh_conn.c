...
static void hidh_l2cif_disconnect_ind(uint16_t l2cap_cid, uint8_t ack_needed);
...
static void hidh_l2cif_cong_ind(uint16_t l2cap_cid, uint8_t congested);
...
tHID_STATUS hidh_conn_reg(void)
{
    ...
    wm_memset(&hh_cb.l2cap_cfg, 0, sizeof(tL2CAP_CFG_INFO));
                               ...
                               uint16_t data, uint8_t report_id, BT_HDR *buf)
{
    ...
    uint8_t     seg_req = FALSE;
    uint16_t      data_size;
    uint16_t      cid;
    uint16_t      buf_size;
    uint8_t       use_data = 0 ;
    uint8_t     blank_datc = FALSE;
    ...
    do
    {
        if(buf == NULL || blank_datc)
        {
            ...
            if((buf->len > (p_hcon->rem_mtu_size - 1)))
            {
                p_buf = (BT_HDR *)GKI_getbuf(buf_size);
                p_buf->offset = L2CAP_MIN_OFFSET;
                seg_req = TRUE;
        ...
        *p_out++      = HID_BUILD_HDR(trans_type, param);
        ...
        if(seg_req)
        {
            wm_memcpy(p_out, (((uint8_t *)(buf + 1)) + buf->offset), bytes_copied);
        ...
        if(data_size)
        {
            ...
            if(bytes_copied == (p_hcon->rem_mtu_size - 1))
            {
                trans_type = HID_TRANS_DATAC;
                blank_datc = TRUE;
            }
