...
**
*******************************************************************************/
uint8_t SDP_InitDiscoveryDb(tSDP_DISCOVERY_DB *p_db, uint32_t len, uint16_t num_uuid,
...
**
*******************************************************************************/
uint8_t SDP_CancelServiceSearch(tSDP_DISCOVERY_DB *p_db)
{
    #if SDP_CLIENT_ENABLED == TRUE
...
**
*******************************************************************************/
uint8_t SDP_ServiceSearchRequest(uint8_t *p_bd_addr, tSDP_DISCOVERY_DB *p_db,
...
**
*******************************************************************************/
uint8_t SDP_ServiceSearchAttributeRequest(uint8_t *p_bd_addr, tSDP_DISCOVERY_DB *p_db,
        tSDP_DISC_CMPL_CB *p_cb)
{
    #if SDP_CLIENT_ENABLED == TRUE
    tCONN_CB     *p_ccb;
...
**
*******************************************************************************/
uint8_t SDP_ServiceSearchAttributeRequest2(uint8_t *p_bd_addr, tSDP_DISCOVERY_DB *p_db,
...
**
*******************************************************************************/
uint8_t SDP_FindServiceUUIDInRec(tSDP_DISC_REC *p_rec, tBT_UUID *p_uuid)
{
    #if SDP_CLIENT_ENABLED == TRUE
                ...
                if(SDP_DISC_ATTR_TYPE(p_sattr->attr_len_type) == UUID_DESC_TYPE)
                {
                    if(SDP_DISC_ATTR_LEN(p_sattr->attr_len_type) == LEN_UUID_16)
                    {
                        ...
                        if(SDP_DISC_ATTR_LEN(p_sattr->attr_len_type) == LEN_UUID_128)
                        {
                            ...
                            if(SDP_DISC_ATTR_LEN(p_sattr->attr_len_type) == LEN_UUID_32)
                            {
...
**
*******************************************************************************/
uint8_t SDP_FindServiceUUIDInRec_128bit(tSDP_DISC_REC *p_rec, tBT_UUID *p_uuid)
{
    #if SDP_CLIENT_ENABLED == TRUE
...
**
*******************************************************************************/
static uint8_t sdp_fill_proto_elem(tSDP_DISC_ATTR  *p_attr, uint16_t layer_uuid,
...
**
*******************************************************************************/
uint8_t SDP_FindProtocolListElemInRec(tSDP_DISC_REC *p_rec, uint16_t layer_uuid, tSDP_PROTOCOL_ELEM *p_elem)
{
    #if SDP_CLIENT_ENABLED == TRUE
    tSDP_DISC_ATTR  *p_attr;
    p_attr = p_rec->p_first_attr;
...
**
*******************************************************************************/
uint8_t SDP_FindAddProtoListsElemInRec(tSDP_DISC_REC *p_rec, uint16_t layer_uuid, tSDP_PROTOCOL_ELEM *p_elem)
{
    #if SDP_CLIENT_ENABLED == TRUE
    tSDP_DISC_ATTR  *p_attr, *p_sattr;
    uint8_t         ret = FALSE;
...
**
*******************************************************************************/
uint8_t SDP_FindProfileVersionInRec(tSDP_DISC_REC *p_rec, uint16_t profile_uuid, uint16_t *p_version)
{
    #if SDP_CLIENT_ENABLED == TRUE
    tSDP_DISC_ATTR  *p_attr, *p_sattr;
    p_attr = p_rec->p_first_attr;
...
static void SDP_AttrStringCopy(char *dst, tSDP_DISC_ATTR *p_attr, uint16_t dst_size)
{
    ...
    if(p_attr)
    {
        ...
        wm_memcpy(dst, (char *)p_attr->attr_value.v.array, len);
        dst[len] = '\0';
    }
    else
    {
                         ...
                         tSDP_DISCOVERY_DB *p_db)
{
    ...
    if(result == SDP_SUCCESS)
    {
        ...
        if(p_curr_attr)
        {
            p_device_info->rec.primary_record = (uint8_t)p_curr_attr->attr_value.v.u8;
        }
        else
        {
            result = SDP_ERR_ATTR_NOT_PRESENT;
