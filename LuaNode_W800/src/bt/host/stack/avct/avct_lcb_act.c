...
static BT_HDR *avct_lcb_msg_asmbl(tAVCT_LCB *p_lcb, BT_HDR *p_buf)
{
    ...
    /* single packet */
        ...
        if(pkt_type == AVCT_PKT_TYPE_SINGLE)
        {
            ...
            GKI_free_and_reset_buf((void **)&p_lcb->p_rx_msg);
            p_ret = p_buf;
        }
        /* start packet */
        else
            if(pkt_type == AVCT_PKT_TYPE_START)
            {
                 ...
                 */
                p_lcb->p_rx_msg = (BT_HDR *)GKI_getbuf(BT_DEFAULT_BUFFER_SIZE);
                wm_memcpy(p_lcb->p_rx_msg, p_buf,
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        wm_memcpy((uint8_t *)(p_lcb->p_rx_msg + 1) + p_lcb->p_rx_msg->offset,
    ...
    for(i = 0; i < AVCT_NUM_CONN; i++, p_ccb++)
    {
        ...
        if(p_ccb->allocated)
        {
            ...
            /* if unbound acceptor and lcb doesn't already have a ccb for this PID */
                ...
                if((p_ccb->p_lcb == NULL) && (p_ccb->cc.role == AVCT_ACP) &&
...
void avct_lcb_discard_msg(tAVCT_LCB *p_lcb, tAVCT_LCB_EVT *p_data)
{
    ...
    GKI_free_and_reset_buf((void **)&p_data->ul_msg.p_buf);
}
...
void avct_lcb_free_msg_ind(tAVCT_LCB *p_lcb, tAVCT_LCB_EVT *p_data)
{
    ...
    GKI_free_and_reset_buf((void **)&p_data->p_buf);
}
