...
typedef struct
{
     ...
     */
    TIMER_LIST_ENT         idle_ccb_timer; /* Idle CCB timer entry */
    TIMER_LIST_ENT         ret_ccb_timer; /* Ret CCB timer entry */
    TIMER_LIST_ENT         rsp_ccb_timer; /* Rsp CCB timer entry */
...
typedef struct
{
    ...
    TIMER_LIST_ENT  transport_channel_timer; /* transport channel connect timer */
    ...
    uint8_t         allocated;      /* whether scb is allocated or unused */
    uint8_t         in_use;         /* whether stream being used by peer */
    uint8_t           role;           /* initiator/acceptor role in current procedure */
    uint8_t         remove;         /* whether CB is marked for removal */
...
extern uint8_t avdt_msg_send(tAVDT_CCB *p_ccb, BT_HDR *p_msg);
...
extern void avdt_ad_tc_cong_ind(tAVDT_TC_TBL *p_tbl, uint8_t is_congested);
