...
void btm_pm_sm_alloc(uint8_t ind)
{
    ...
    wm_memset(p_db, 0, sizeof(tBTM_PM_MCB));
...
static tBTM_STATUS btm_pm_snd_md_req(uint8_t pm_id, int link_ind, tBTM_PM_PWR_MD *p_mode)
{
    ...
    if(p_cb->chg_ind == TRUE)   /* needs to wake first */
    {
        ...
        if(BTM_PM_MD_SNIFF == md_res.mode && p_cb->max_lat)
        {
...
**
*******************************************************************************/
uint8_t btm_pm_device_in_active_or_sniff_mode(void)
{
...
**
*******************************************************************************/
uint8_t btm_pm_device_in_scan_state(void)
{
