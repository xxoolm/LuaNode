...
extern uint8_t aes_cipher_msg_auth_code(BT_OCTET16 key, uint8_t *input, uint16_t length,
                                        uint16_t tlen, uint8_t *p_signature);
extern void smp_link_encrypted(BD_ADDR bda, uint8_t encr_enable);
extern uint8_t smp_proc_ltk_request(BD_ADDR bda);
#endif
...
**
*******************************************************************************/
uint8_t BTM_SecAddBleDevice(BD_ADDR bd_addr, BD_NAME bd_name, tBT_DEVICE_TYPE dev_type,
                            tBLE_ADDR_TYPE addr_type)
{
    ...
    wm_memcpy(p_dev_rec->ble.pseudo_addr, bd_addr, BD_ADDR_LEN);
...
**
*******************************************************************************/
uint8_t BTM_SecAddBleKey(BD_ADDR bd_addr, tBTM_LE_KEY_VALUE *p_le_key, tBTM_LE_KEY_TYPE key_type)
{
    #if SMP_INCLUDED == TRUE
    ...
    if(p_key != NULL)
    {
        switch(key_type)
        {
            case BTM_BLE_KEY_TYPE_ID:
                wm_memcpy(&p_devcb->id_keys, &p_key->id_keys, sizeof(tBTM_BLE_LOCAL_ID_KEYS));
                break;

            case BTM_BLE_KEY_TYPE_ER:
                wm_memcpy(p_devcb->ble_encryption_key_value, p_key->er, sizeof(BT_OCTET16));
                break;
...
{
    BTM_TRACE_DEBUG("%s", __func__);
    wm_memcpy(ble_encr_key_value, btm_cb.devcb.ble_encryption_key_value, BT_OCTET16_LEN);
}
...
void BTM_GetDeviceIDRoot(BT_OCTET16 irk)
{
    ...
    wm_memcpy(irk, btm_cb.devcb.id_keys.irk, BT_OCTET16_LEN);
}
...
void BTM_GetDeviceDHK(BT_OCTET16 dhk)
{
    ...
    wm_memcpy(dhk, btm_cb.devcb.id_keys.dhk, BT_OCTET16_LEN);
}
...
void BTM_ReadConnectionAddr(BD_ADDR remote_bda, BD_ADDR local_conn_addr, tBLE_ADDR_TYPE *p_addr_type)
{
    ...
    wm_memcpy(local_conn_addr, p_acl->conn_addr, BD_ADDR_LEN);
...
**
*******************************************************************************/
uint8_t BTM_IsBleConnection(uint16_t conn_handle)
{
    #if (BLE_INCLUDED == TRUE)
    uint8_t                xx;
    tACL_CONN            *p;
...
**
** Returns          uint8_t , TRUE if connection to remote device exists, else FALSE
**
*******************************************************************************/
uint8_t BTM_ReadRemoteConnectionAddr(BD_ADDR pseudo_addr, BD_ADDR conn_addr,
                                     tBLE_ADDR_TYPE *p_addr_type)
{
    ...
    wm_memcpy(conn_addr, p->active_remote_addr, BD_ADDR_LEN);
    *p_addr_type = p->active_remote_addr_type;
    #else
    tBTM_SEC_DEV_REC *p_dev_rec = btm_find_dev(pseudo_addr);
    wm_memcpy(conn_addr, pseudo_addr, BD_ADDR_LEN);
    ...
    else /* there is a security device record exisitng */
    {
                ...
                memcmp(p_dev_rec->ble.pseudo_addr, remote_bda, BD_ADDR_LEN) == 0)
        {
            ...
            *p_addr_type = p_dev_rec->ble.ble_addr_type;
            ...
            if(memcmp(p_dev_rec->ble.pseudo_addr, remote_bda, BD_ADDR_LEN) == 0)
            {
...
**
*******************************************************************************/
uint8_t BTM_ReadConnectedTransportAddress(BD_ADDR remote_bda, tBT_TRANSPORT transport)
{
    tBTM_SEC_DEV_REC *p_dev_rec = btm_find_dev(remote_bda);
    ...
    if(transport == BT_TRANSPORT_BR_EDR)
    {
        ...
        {
            wm_memcpy(remote_bda, p_dev_rec->bd_addr, BD_ADDR_LEN);
            return TRUE;
        }
        else
            if(p_dev_rec->device_type & BT_DEVICE_TYPE_BREDR)
            {
                wm_memcpy(remote_bda, p_dev_rec->bd_addr, BD_ADDR_LEN);
            }
            else
            {
                wm_memset(remote_bda, 0, BD_ADDR_LEN);
            }
    ...
    {
        wm_memcpy(remote_bda, p_dev_rec->ble.pseudo_addr, BD_ADDR_LEN);
...
**
*******************************************************************************/
uint8_t BTM_UseLeLink(BD_ADDR bd_addr)
{
    tACL_CONN         *p;
    tBT_DEVICE_TYPE     dev_type;
    tBLE_ADDR_TYPE      addr_type;
    uint8_t             use_le = FALSE;
    ...
    if((p = btm_bda_to_acl(bd_addr, BT_TRANSPORT_BR_EDR)) != NULL)
    {
        ...
        if((p = btm_bda_to_acl(bd_addr, BT_TRANSPORT_LE)) != NULL)
        {
            use_le = TRUE;
        }
        else
...
tBTM_STATUS BTM_SetBleDataLength(BD_ADDR bd_addr, uint16_t tx_pdu_length)
{
    ...
    if(p_acl != NULL)
    {
        if(tx_pdu_length > BTM_BLE_DATA_SIZE_MAX)
        {
            ...
            if(tx_pdu_length < BTM_BLE_DATA_SIZE_MIN)
            {
...
**
*******************************************************************************/
tBTM_SEC_ACTION btm_ble_determine_security_act(uint8_t is_originator, BD_ADDR bdaddr, uint16_t security_required)
{
    ...
    uint8_t is_link_encrypted = FALSE;
    uint8_t is_key_mitm = FALSE;
...
**
*******************************************************************************/
uint8_t btm_ble_start_sec_check(BD_ADDR bd_addr, uint16_t psm, uint8_t is_originator,
...
void btm_ble_rand_enc_complete(uint8_t *p, uint16_t op_code, tBTM_RAND_ENC_CB *p_enc_cplt_cback)
{
    ...
    wm_memset(&params, 0, sizeof(tBTM_RAND_ENC));
    ...
    if(p_enc_cplt_cback && p)
    {
        ...
        if(params.status == HCI_SUCCESS)
        {
            ...
            wm_memcpy(p_dest, p, params.param_len);   /* Fetch return info from HCI event message */
        }
...
**
*******************************************************************************/
void btm_ble_increment_sign_ctr(BD_ADDR bd_addr, uint8_t is_local)
{
    tBTM_SEC_DEV_REC *p_dev_rec;
...
**
*******************************************************************************/
uint8_t btm_ble_get_enc_key_type(BD_ADDR bd_addr, uint8_t *p_key_types)
{
    tBTM_SEC_DEV_REC *p_dev_rec;
...
*******************************************************************************/
uint8_t btm_get_local_div(BD_ADDR bd_addr, uint16_t *p_div)
{
    tBTM_SEC_DEV_REC   *p_dev_rec;
    uint8_t            status = FALSE;
    ...
    if((p_rec = btm_find_dev(bd_addr)) != NULL && (p_keys || key_type == BTM_LE_KEY_LID))
    {
        ...
        switch(key_type)
        {
            case BTM_LE_KEY_PENC:
                wm_memcpy(p_rec->ble.keys.pltk, p_keys->penc_key.ltk, BT_OCTET16_LEN);
                wm_memcpy(p_rec->ble.keys.rand, p_keys->penc_key.rand, BT_OCTET8_LEN);
            ...
            case BTM_LE_KEY_PID:
                ...
                //wm_memcpy( p_rec->ble.keys.irk, p_keys->pid_key, BT_OCTET16_LEN); todo will crash the system
                wm_memcpy(p_rec->ble.static_addr, p_keys->pid_key.static_addr, BD_ADDR_LEN);
                ...
                wm_memcpy(p_rec->bd_addr, p_keys->pid_key.static_addr, BD_ADDR_LEN);
                /* combine DUMO device security record if needed */
                btm_consolidate_dev(p_rec);
                break;

            case BTM_LE_KEY_PCSRK:
                wm_memcpy(p_rec->ble.keys.pcsrk, p_keys->pcsrk_key.csrk, BT_OCTET16_LEN);
            ...
            case BTM_LE_KEY_LENC:
                wm_memcpy(p_rec->ble.keys.lltk, p_keys->lenc_key.ltk, BT_OCTET16_LEN);
            ...
            case BTM_LE_KEY_LCSRK:/* local CSRK has been delivered */
                wm_memcpy(p_rec->ble.keys.lcsrk, p_keys->lcsrk_key.csrk, BT_OCTET16_LEN);
...
void btm_ble_ltk_request(uint16_t handle, uint8_t rand[8], uint16_t ediv)
{
    ...
    p_cb->ediv = ediv;
    wm_memcpy(p_cb->enc_rand, rand, BT_OCTET8_LEN);
...
**
*******************************************************************************/
tBTM_STATUS btm_ble_start_encrypt(BD_ADDR bda, uint8_t use_stk, BT_OCTET16 stk)
{
    tBTM_CB *p_cb = &btm_cb;
...
**
*******************************************************************************/
void btm_ble_ltk_request_reply(BD_ADDR bda,  uint8_t use_stk, BT_OCTET16 stk)
{
    tBTM_SEC_DEV_REC    *p_rec = btm_find_dev(bda);
    tBTM_CB *p_cb = &btm_cb;
                       ...
                       tBLE_ADDR_TYPE addr_type, uint8_t addr_matched)
{
    ...
    */
    ...
    wm_memcpy(p_dev_rec->ble.pseudo_addr, bda, BD_ADDR_LEN);
    ...
    {
        wm_memcpy(p_dev_rec->ble.cur_rand_addr, bda, BD_ADDR_LEN);
    }
...
**
******************************************************************************/
void btm_ble_conn_complete(uint8_t *p, uint16_t evt_len, uint8_t enhanced)
{
    #if (BLE_PRIVACY_SPT == TRUE )
...
**
*******************************************************************************/
uint8_t BTM_BleDataSignature(BD_ADDR bd_addr, uint8_t *p_text, uint16_t len,
                             BLE_SIGNATURE signature)
{
    ...
    else
    {
        ...
        uint8_t *pp;
        uint8_t *p_buf = (uint8_t *)GKI_getbuf(len + 4);
...
**
*******************************************************************************/
uint8_t BTM_BleVerifySignature(BD_ADDR bd_addr, uint8_t *p_orig, uint16_t len, uint32_t counter,
                               uint8_t *p_comp)
{
    uint8_t verified = FALSE;
    #if SMP_INCLUDED == TRUE
    tBTM_SEC_DEV_REC *p_rec = btm_find_dev(bd_addr);
    ...
    if(p_rec == NULL || (p_rec && !(p_rec->ble.key_type & BTM_LE_KEY_PCSRK)))
    {
        ...
        if(counter < p_rec->ble.keys.counter)
        {
...
**
*******************************************************************************/
uint8_t BTM_GetLeSecurityState(BD_ADDR bd_addr, uint8_t *p_le_dev_sec_flags, uint8_t *p_le_key_size)
{
    #if (BLE_INCLUDED == TRUE)
    tBTM_SEC_DEV_REC *p_dev_rec;
...
**
*******************************************************************************/
uint8_t BTM_BleSecurityProcedureIsRunning(BD_ADDR bd_addr)
{
    #if (BLE_INCLUDED == TRUE)
    tBTM_SEC_DEV_REC *p_dev_rec = btm_find_dev(bd_addr);
...
static void btm_ble_process_er2(tBTM_RAND_ENC *p)
{
    ...
    {
        wm_memcpy(&btm_cb.devcb.ble_encryption_key_value[8], p->param_buf, BT_OCTET8_LEN);
        btm_notify_new_key(BTM_BLE_KEY_TYPE_ER);
    }
    else
    {
        BTM_TRACE_ERROR("Generating ER2 exception.");
        wm_memset(&btm_cb.devcb.ble_encryption_key_value, 0, sizeof(BT_OCTET16));
    }
}
...
static void btm_ble_process_er(tBTM_RAND_ENC *p)
{
    ...
    if(p && p->opcode == HCI_BLE_RAND)
    {
        wm_memcpy(&btm_cb.devcb.ble_encryption_key_value[0], p->param_buf, BT_OCTET8_LEN);
...
static void btm_ble_process_irk(tSMP_ENC *p)
{
    ...
    {
        wm_memcpy(btm_cb.devcb.id_keys.irk, p->param_buf, BT_OCTET16_LEN);
...
static void btm_ble_process_dhk(tSMP_ENC *p)
{
    ...
    if(p && p->opcode == HCI_BLE_ENCRYPT)
    {
        wm_memcpy(btm_cb.devcb.id_keys.dhk, p->param_buf, BT_OCTET16_LEN);
                        ...
                        1,   &output))
        {
            ...
            wm_memset(&btm_cb.devcb.id_keys, 0, sizeof(tBTM_BLE_LOCAL_ID_KEYS));
        }
        else
        {
...
static void btm_ble_process_ir2(tBTM_RAND_ENC *p)
{
    ...
    if(p && p->opcode == HCI_BLE_RAND)
    {
        ...
        wm_memcpy(&btm_cb.devcb.id_keys.ir[8], p->param_buf, BT_OCTET8_LEN);
...
static void btm_ble_process_ir(tBTM_RAND_ENC *p)
{
    ...
    {
        /* remembering in control block */
        wm_memcpy(btm_cb.devcb.id_keys.ir, p->param_buf, BT_OCTET8_LEN);
...
**
*******************************************************************************/
void btm_ble_set_no_disc_if_pair_fail(uint8_t disable_disc)
{
...
**
*******************************************************************************/
void btm_ble_set_test_mac_value(uint8_t enable, uint8_t *p_test_mac_val)
{
    BTM_TRACE_DEBUG("btm_ble_set_test_mac_value enable=%d", enable);
    btm_cb.devcb.enable_test_mac_val = enable;
    wm_memcpy(btm_cb.devcb.test_mac, p_test_mac_val, BT_OCTET8_LEN);
}
...
**
*******************************************************************************/
void btm_ble_set_test_local_sign_cntr_value(uint8_t enable, uint32_t test_local_sign_cntr)
{
...
void btm_set_random_address(BD_ADDR random_bda)
{
    ...
    uint8_t     adv_mode = btm_cb.ble_ctr_cb.inq_var.adv_mode ;
    BTM_TRACE_DEBUG("btm_set_random_address, adv_mode=%d\r\n", adv_mode);
    ...
    wm_memcpy(p_cb->private_addr, random_bda, BD_ADDR_LEN);
...
**
*******************************************************************************/
void btm_ble_set_keep_rfu_in_auth_req(uint8_t keep_rfu)
{
