...
void btm_ble_batchscan_filter_track_adv_vse_cback(uint8_t len, uint8_t *p)
{
    ...
    if(HCI_VSE_SUBCODE_BLE_TRACKING_SUB_EVT == sub_event && NULL != ble_advtrack_cb.p_track_cback)
    {
        ...
        wm_memset(&adv_data, 0, sizeof(tBTM_BLE_TRACK_ADV_DATA));
        ...
        tBTM_BLE_REF_VALUE ref_value)
{
    ...
    pp = param;
    wm_memset(param, 0, BTM_BLE_BATCH_SCAN_READ_RESULTS_LEN);
                                       ...
                                       uint8_t batch_scan_notify_threshold)
{
    ...
    pp = param;
    wm_memset(param, 0, BTM_BLE_BATCH_SCAN_STORAGE_CFG_LEN);
                                        ...
                                        tBTM_BLE_DISCARD_RULE discard_rule)
{
    ...
    pp_scan = scan_param;
    wm_memset(scan_param, 0, BTM_BLE_BATCH_SCAN_PARAM_CONFIG_LEN);
...
**
*******************************************************************************/
tBTM_STATUS btm_ble_enable_disable_batchscan(uint8_t should_enable)
{
    tBTM_STATUS     status = BTM_NO_RESOURCES;
...
void btm_ble_batchscan_init(void)
{
    ...
    wm_memset(&ble_batchscan_cb, 0, sizeof(tBTM_BLE_BATCH_SCAN_CB));
    wm_memset(&ble_advtrack_cb, 0, sizeof(tBTM_BLE_ADV_TRACK_CB));
