...
#define BTM_SEC_MAX_COLLISION_DELAY     (5000)
    ...
    uint8_t (APPL_AUTH_WRITE_EXCEPTION)(BD_ADDR bd_addr);
#endif

/********************************************************************************
**              L O C A L    F U N C T I O N     P R O T O T Y P E S            *
*********************************************************************************/
tBTM_SEC_SERV_REC *btm_sec_find_first_serv(uint8_t is_originator, uint16_t psm);
...
static uint8_t  btm_sec_start_get_name(tBTM_SEC_DEV_REC *p_dev_rec);
static uint8_t  btm_sec_start_authentication(tBTM_SEC_DEV_REC *p_dev_rec);
static uint8_t  btm_sec_start_encryption(tBTM_SEC_DEV_REC *p_dev_rec);
...
static uint8_t  btm_sec_queue_mx_request(BD_ADDR bd_addr,  uint16_t psm,  uint8_t is_orig,
        uint32_t mx_proto_id, uint32_t mx_chan_id,
        tBTM_SEC_CALLBACK *p_callback, void *p_ref_data);
static void     btm_sec_bond_cancel_complete(void);
static void     btm_send_link_key_notif(tBTM_SEC_DEV_REC *p_dev_rec);
static uint8_t  btm_sec_check_prefetch_pin(tBTM_SEC_DEV_REC  *p_dev_rec);

static uint8_t    btm_sec_start_authorization(tBTM_SEC_DEV_REC *p_dev_rec);
uint8_t         btm_sec_are_all_trusted(uint32_t p_mask[]);
...
static uint8_t  btm_sec_set_security_level(CONNECTION_TYPE conn_type, char *p_name, uint8_t service_id,
        uint16_t sec_level, uint16_t psm, uint32_t mx_proto_id,
        uint32_t mx_chan_id);

static uint8_t btm_dev_authenticated(tBTM_SEC_DEV_REC *p_dev_rec);
static uint8_t btm_dev_encrypted(tBTM_SEC_DEV_REC *p_dev_rec);
static uint8_t btm_dev_authorized(tBTM_SEC_DEV_REC *p_dev_rec);
static uint8_t btm_serv_trusted(tBTM_SEC_DEV_REC *p_dev_rec, tBTM_SEC_SERV_REC *p_serv_rec);
static uint8_t btm_sec_is_serv_level0(uint16_t psm);
static uint16_t  btm_sec_set_serv_level4_flags(uint16_t cur_security, uint8_t is_originator);

static uint8_t btm_sec_queue_encrypt_request(BD_ADDR bd_addr, tBT_TRANSPORT transport,
        tBTM_SEC_CALLBACK *p_callback, void *p_ref_data,
        tBTM_BLE_SEC_ACT sec_act);
static void btm_sec_check_pending_enc_req(tBTM_SEC_DEV_REC  *p_dev_rec, tBT_TRANSPORT transport,
        uint8_t encr_enable);

static uint8_t btm_sec_use_smp_br_chnl(tBTM_SEC_DEV_REC *p_dev_rec);
static uint8_t btm_sec_is_master(tBTM_SEC_DEV_REC *p_dev_rec);

/* TRUE - authenticated link key is possible */
static const uint8_t btm_sec_io_map [BTM_IO_CAP_MAX][BTM_IO_CAP_MAX] =
{
    /*   OUT,    IO,     IN,     NONE */
...
**
*******************************************************************************/
static uint8_t btm_dev_16_digit_authenticated(tBTM_SEC_DEV_REC *p_dev_rec)
{
...
**
*******************************************************************************/
uint8_t BTM_SecRegister(tBTM_APPL_INFO *p_cb_info)
{
    #if BLE_INCLUDED == TRUE
	...
	memcpy(&btm_cb.api, p_cb_info, sizeof(tBTM_APPL_INFO));
    #if BLE_INCLUDED == TRUE && SMP_INCLUDED == TRUE
    BTM_TRACE_EVENT("%s btm_cb.api.p_le_callback = 0x%p \r\n", __func__, btm_cb.api.p_le_callback);
    #endif
    BTM_TRACE_EVENT("%s application registered", __func__);
    return(TRUE);
}
...
**
*******************************************************************************/
uint8_t BTM_SecRegisterLinkKeyNotificationCallback(tBTM_LINK_KEY_CALLBACK *p_callback)
{
...
**
*******************************************************************************/
uint8_t  BTM_SecAddRmtNameNotifyCallback(tBTM_RMT_NAME_CALLBACK *p_callback)
{
    int i;
...
**
*******************************************************************************/
uint8_t  BTM_SecDeleteRmtNameNotifyCallback(tBTM_RMT_NAME_CALLBACK *p_callback)
{
    int i;
...
**
** Returns          uint8_t TRUE or FALSE is device found
**
*******************************************************************************/
uint8_t BTM_GetSecurityFlags(BD_ADDR bd_addr, uint8_t *p_sec_flags)
{
...
**
*******************************************************************************/
uint8_t BTM_GetSecurityFlagsByTransport(BD_ADDR bd_addr, uint8_t *p_sec_flags,
...
void BTM_SetPinType(uint8_t pin_type, PIN_CODE pin_code, uint8_t pin_code_len)
{
    ...
    wm_memcpy(btm_cb.cfg.pin_code, pin_code, pin_code_len);
}
...
**
*******************************************************************************/
void BTM_SetPairableMode(uint8_t allow_pairing, uint8_t connect_only_paired)
{
...
**
*******************************************************************************/
void BTM_SetSecureConnectionsOnly(uint8_t secure_connections_only_mode)
{
...
**
*******************************************************************************/
uint8_t BTM_SetSecurityLevel(uint8_t is_originator, char *p_name, uint8_t service_id,
                             uint16_t sec_level, uint16_t psm, uint32_t mx_proto_id,
                             uint32_t mx_chan_id)
{
        ...
        uint32_t mx_chan_id)
{
    ...
    uint8_t              record_allocated = FALSE;
    ...
    for(index = 0; index < BTM_SEC_MAX_SERVICE_RECORDS; index++, p_srec++)
    {
        ...
        /* Mark the first available service record */
            ...
            if(!record_allocated)
            {
                wm_memset(p_srec, 0, sizeof(tBTM_SEC_SERV_REC));
                record_allocated = TRUE;
...
void BTM_PINCodeReply(BD_ADDR bd_addr, uint8_t res, uint8_t pin_len, uint8_t *p_pin, uint32_t trusted_mask[])
{
            ...
            && (btm_cb.security_mode_changed == FALSE))
    {
        ...
        wm_memcpy(btm_cb.pin_code, p_pin, pin_len);
                                      ...
                                      uint8_t pin_len, uint8_t *p_pin, uint32_t trusted_mask[])
{
    ...
    if(p_pin && (pin_len <= PIN_CODE_LEN) && (pin_len != 0))
    {
        btm_cb.pin_code_len = pin_len;
        p_dev_rec->pin_code_length = pin_len;
        wm_memcpy(btm_cb.pin_code, p_pin, PIN_CODE_LEN);
    }

    wm_memcpy(btm_cb.pairing_bda, bd_addr, BD_ADDR_LEN);
    btm_cb.pairing_flags = BTM_PAIR_FLAGS_WE_STARTED_DD;
...
tBTM_STATUS BTM_SecGetDeviceLinkKey(BD_ADDR bd_addr, LINK_KEY link_key)
{
    ...
    {
        wm_memcpy(link_key, p_dev_rec->link_key, LINK_KEY_LEN);
        return(BTM_SUCCESS);
    }
...
**
*******************************************************************************/
uint8_t BTM_BothEndsSupportSecureConnections(BD_ADDR bd_addr)
{
    return ((HCI_SC_CTRLR_SUPPORTED(btm_cb.devcb.local_lmp_features[HCI_EXT_FEATURES_PAGE_2])) &&
...
**
*******************************************************************************/
uint8_t BTM_PeerSupportsSecureConnections(BD_ADDR bd_addr)
{
    tBTM_SEC_DEV_REC    *p_dev_rec;
...
**
*******************************************************************************/
static uint8_t btm_sec_is_upgrade_possible(tBTM_SEC_DEV_REC  *p_dev_rec, uint8_t is_originator)
{
    uint16_t              mtm_check = is_originator ? BTM_SEC_OUT_MITM : BTM_SEC_IN_MITM;
    uint8_t             is_possible = TRUE;
...
**
*******************************************************************************/
static void btm_sec_check_upgrade(tBTM_SEC_DEV_REC  *p_dev_rec, uint8_t is_originator)
{
    ...
    if(btm_sec_is_upgrade_possible(p_dev_rec, is_originator) == TRUE)
    {
        ...
        wm_memcpy(evt_data.bd_addr, p_dev_rec->bd_addr, BD_ADDR_LEN);
                                     ...
                                     void *p_ref_data)
{
    ...
    uint8_t       old_is_originator;
    tBTM_STATUS   rc = BTM_SUCCESS;
    uint8_t       chk_acp_auth_done = FALSE;
    uint8_t is_originator;
    ...
    {
        //uint8_t local_supports_sc = controller_get_interface()->supports_secure_connections();
        uint8_t local_supports_sc = HCI_SC_CTRLR_SUPPORTED(btm_cb.devcb.local_lmp_features[HCI_EXT_FEATURES_PAGE_2]);
...
**
*******************************************************************************/
tBTM_STATUS btm_sec_mx_access_request(BD_ADDR bd_addr, uint16_t psm, uint8_t is_originator,
                                      uint32_t mx_proto_id, uint32_t mx_chan_id,
                                      tBTM_SEC_CALLBACK *p_callback, void *p_ref_data)
{
    tBTM_SEC_DEV_REC  *p_dev_rec;
    ...
    tBTM_STATUS        rc;
    uint16_t             security_required;
    uint8_t transport   = FALSE;/* should check PSM range in LE connection oriented L2CAP connection */
...
void btm_sec_conn_req(uint8_t *bda, uint8_t *dc)
{
    ...
    wm_memcpy(btm_cb.connecting_bda, bda, BD_ADDR_LEN);
    wm_memcpy(btm_cb.connecting_dc,  dc,  DEV_CLASS_LEN);
...
void btm_sec_init(uint8_t sec_mode)
{
    ...
    wm_memset(btm_cb.pairing_bda, 0xff, BD_ADDR_LEN);
...
void btm_sec_dev_reset(void)
{
    if(HCI_SIMPLE_PAIRING_SUPPORTED(btm_cb.devcb.local_lmp_features[HCI_EXT_FEATURES_PAGE_0]))
    {
        ...
        btsnd_hcic_set_event_mask(LOCAL_BR_EDR_CONTROLLER_ID,
                                  (uint8_t *)HCI_DUMO_EVENT_MASK_EXT);
        btsnd_hcic_ble_set_evt_mask((uint8_t *)HCI_BLE_EVENT_MASK_DEF);
        #else
        btsnd_hcic_set_event_mask(LOCAL_BR_EDR_CONTROLLER_ID,
                                  (uint8_t *)HCI_LISBON_EVENT_MASK_EXT);
        #endif
...
uint8_t is_state_getting_name(void *data, void *context)
{
...
void btm_io_capabilities_req(uint8_t *p)
{
    ...
    wm_memcpy(btm_cb.pairing_bda, evt_data.bd_addr, BD_ADDR_LEN);
...
void btm_proc_sp_req_evt(tBTM_SP_EVT event, uint8_t *p)
{
    ...
    {
        wm_memcpy(evt_data.cfm_req.bd_addr, p_dev_rec->bd_addr, BD_ADDR_LEN);
        wm_memcpy(evt_data.cfm_req.dev_class, p_dev_rec->dev_class, DEV_CLASS_LEN);
        ...
        if(btm_cb.api.p_sp_callback)
        {
            ...
            if((event == BTM_SP_CFM_REQ_EVT) && (evt_data.cfm_req.just_works == TRUE))
            {
...
void btm_simple_pair_complete(uint8_t *p)
{
    ...
    else
    {
        if(status == HCI_ERR_PAIRING_NOT_ALLOWED)
        {
            ...
            #else
            btm_cb.pairing_timer.p_cback = (TIMER_CBACK *)&btm_sec_pairing_timeout;
            btm_cb.pairing_timer.param = (TIMER_PARAM_TYPE)NULL;
            btu_start_timer(&btm_cb.pairing_timer, BTU_TTYPE_USER_FUNC, BT_1SEC_TIMEOUT_MS / 1000);
            #endif
        }
        else
            if(memcmp(btm_cb.pairing_bda, evt_data.bd_addr, BD_ADDR_LEN) == 0)
            {
                /* stop the timer */
                #ifdef USE_ALARM
                alarm_cancel(btm_cb.pairing_timer);
                #else
                btu_stop_timer(&btm_cb.pairing_timer);
                #endif
    ...
    wm_memcpy(evt_data.bd_addr, p_dev_rec->bd_addr, BD_ADDR_LEN);
    wm_memcpy(evt_data.dev_class, p_dev_rec->dev_class, DEV_CLASS_LEN);
...
static void btm_sec_auth_collision(uint16_t handle)
{
    ...
    {
        btm_cb.collision_start_time = GKI_time_get_os_boottime_ms();
    }

    if((GKI_time_get_os_boottime_ms() - btm_cb.collision_start_time) < btm_cb.max_collision_delay)
    {
        ...
        if(p_dev_rec != NULL)
        {
            ...
            alarm_set_on_queue(btm_cb.sec_collision_timer, BT_1SEC_TIMEOUT_MS,
                               ...
                               btu_general_alarm_queue);
            #else
            btm_cb.sec_collision_timer.p_cback = (TIMER_CBACK *)&btm_sec_collision_timeout;
            btm_cb.sec_collision_timer.param = (TIMER_PARAM_TYPE)NULL;
            btu_start_timer(&btm_cb.sec_collision_timer, BTU_TTYPE_USER_FUNC, BT_1SEC_TIMEOUT_MS / 1000);
            #endif
        }
    }
}
    ...
    if(status != HCI_SUCCESS)
    {
        if((old_sm4 & BTM_SM4_RETRY) == 0)
        {
             ...
             * Legacy device do not need this. the controller will drive the retry automatically */
                ...
                if(HCI_ERR_KEY_MISSING == status && BTM_SEC_IS_SM4(p_dev_rec->sm4))
                {
                    /* not retried yet. set the retry bit */
                    p_dev_rec->sm4 |= BTM_SM4_RETRY;
                    p_dev_rec->sec_flags &= ~BTM_SEC_LINK_KEY_KNOWN;
...
void btm_sec_connected(uint8_t *bda, uint16_t handle, uint8_t status, uint8_t enc_mode)
{
    tBTM_SEC_DEV_REC *p_dev_rec = btm_find_dev(bda);
    uint8_t            res;
    uint8_t          is_pairing_device = FALSE;
    tACL_CONN        *p_acl_cb;
    uint8_t            bit_shift = 0;
    btm_acl_resubmit_page();
    /* Commenting out trace due to obf/compilation problems.
    */
                    ...
                    && (btm_cb.pairing_flags & BTM_PAIR_FLAGS_WE_STARTED_DD))
            {
                ...
                if((status != HCI_SUCCESS) && (btm_cb.pairing_state == BTM_PAIR_STATE_WAIT_PIN_REQ))
                {
                    ...
                    if(p_dev_rec->sec_flags & BTM_SEC_NAME_KNOWN)
                    {
                        ...
                        #else
                        btm_cb.sec_collision_timer.param = (TIMER_PARAM_TYPE)NULL;
                        btm_cb.sec_collision_timer.p_cback = (TIMER_CBACK *)&btm_sec_connect_after_reject_timeout;
                        btu_start_timer(&btm_cb.sec_collision_timer, BTU_TTYPE_USER_FUNC, 0);
                        #endif
                    }
                    else
                    {
    ...
    if(status != HCI_SUCCESS)
    {
         ...
         */
            ...
            if((p_dev_rec->link_key_type  <= BTM_LKEY_TYPE_REMOTE_UNIT) &&
            ...
            && (memcmp(btm_cb.pairing_bda, p_dev_rec->bd_addr, BD_ADDR_LEN) == 0))
    {
        ...
        if(btm_cb.api.p_auth_complete_callback)
        {
            ...
            if(reason == HCI_ERR_REPEATED_ATTEMPTS)
            {
                ...
                if(old_pairing_flags & BTM_PAIR_FLAGS_WE_STARTED_DD)
                {
...
void btm_sec_link_key_notification(uint8_t *p_bda, uint8_t *p_link_key, uint8_t key_type)
{
    ...
    uint8_t         we_are_bonding = FALSE;
    uint8_t         ltk_derived_lk  = FALSE;
    ...
    p_dev_rec->enc_key_size = 16;
    #endif
    wm_memcpy(p_dev_rec->link_key, p_link_key, LINK_KEY_LEN);
...
void btm_sec_pin_code_request(uint8_t *p_bda)
{
    ...
    if(btm_cb.pairing_state == BTM_PAIR_STATE_IDLE)
    {
        wm_memcpy(btm_cb.pairing_bda, p_bda, BD_ADDR_LEN);
        btm_cb.pairing_flags = BTM_PAIR_FLAGS_PEER_STARTED_DD;
        /* Make sure we reset the trusted mask to help against attacks */
        BTM_SEC_CLR_TRUSTED_DEVICE(p_dev_rec->trusted_mask);
    }
    ...
    {
        wm_memcpy(p_dev_rec->dev_class, p_cb->connecting_dc, DEV_CLASS_LEN);
    }
    ...
    /* OR we could not allocate entry in the database reject pairing request */
        ...
        else
        {
            ...
            wm_memcpy(p_cb->connecting_bda, p_bda, BD_ADDR_LEN);
            wm_memcpy(p_cb->connecting_dc,  p_dev_rec->dev_class, DEV_CLASS_LEN);
...
uint8_t is_sec_state_equal(void *data, void *context)
{
    tBTM_SEC_DEV_REC *p_dev_rec = data;
...
static void btm_sec_change_pairing_state(tBTM_PAIRING_STATE new_state)
{
    ...
    else
    {
        ...
        btm_cb.pairing_timer.p_cback = (TIMER_CBACK *)&btm_sec_pairing_timeout;
        btm_cb.pairing_timer.param = (TIMER_PARAM_TYPE)NULL;
        btu_start_timer(&btm_cb.pairing_timer, BTU_TTYPE_USER_FUNC, BTM_SEC_TIMEOUT_VALUE);
        #endif
    }
}
...
**
*******************************************************************************/
void btm_sec_dev_rec_cback_event(tBTM_SEC_DEV_REC *p_dev_rec, uint8_t res, uint8_t is_le_transport)
{
                                        ...
                                        tBTM_SEC_CALLBACK *p_callback, void *p_ref_data)
{
    ...
    p_e->sec_act        = 0;
    wm_memcpy(p_e->bd_addr, bd_addr, BD_ADDR_LEN);
    BTM_TRACE_EVENT("%s() PSM: 0x%04x  Is_Orig: %u  mx_proto_id: %u  mx_chan_id: %u",
...
{
    tBTM_SEC_QUEUE_ENTRY *p_e =
                    (tBTM_SEC_QUEUE_ENTRY *)GKI_getbuf(sizeof(tBTM_SEC_QUEUE_ENTRY) + 1);
...
void btm_sec_clear_ble_keys(tBTM_SEC_DEV_REC  *p_dev_rec)
{
    ...
    #if (SMP_INCLUDED== TRUE)
    p_dev_rec->ble.key_type = BTM_LE_KEY_NONE;
    wm_memset(&p_dev_rec->ble.keys, 0, sizeof(tBTM_SEC_BLE_KEYS));
...
**
*******************************************************************************/
uint8_t btm_sec_is_a_bonded_dev(BD_ADDR bda)
{
    tBTM_SEC_DEV_REC *p_dev_rec = btm_find_dev(bda);
    uint8_t is_bonded = FALSE;
...
**
*******************************************************************************/
uint8_t btm_sec_is_le_capable_dev(BD_ADDR bda)
{
    tBTM_SEC_DEV_REC *p_dev_rec = btm_find_dev(bda);
    uint8_t le_capable = FALSE;
    #if (BLE_INCLUDED== TRUE)
