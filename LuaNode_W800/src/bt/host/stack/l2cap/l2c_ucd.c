...
static uint8_t l2c_ucd_connect(BD_ADDR rem_bda);
...
**
*******************************************************************************/
static void l2c_ucd_congestion_status_cback(BD_ADDR rem_bda, uint8_t is_congested)
{
    tL2C_RCB    *p_rcb = &l2cb.rcb_pool[0];
    uint16_t      xx;
...
**
*******************************************************************************/
static void l2c_ucd_disconnect_ind_cback(uint16_t cid, uint8_t result)
{
    /* do nothing */
}
...
**
*******************************************************************************/
uint8_t L2CA_UcdRegister(uint16_t psm, tL2CAP_UCD_CB_INFO *p_cb_info)
{
    tL2C_RCB             *p_rcb;
    ...
    if((p_rcb = l2cu_find_rcb_by_psm(L2C_UCD_RCB_ID)) == NULL)
    {
        ...
        else
        {
            ...
            wm_memset(&p_rcb->api, 0, sizeof(tL2CAP_APPL_INFO));
...
**
*******************************************************************************/
uint8_t L2CA_UcdDeregister(uint16_t psm)
{
    tL2C_CCB    *p_ccb;
    tL2C_RCB    *p_rcb;
    uint16_t      xx;
...
**
*******************************************************************************/
uint8_t L2CA_UcdDiscover(uint16_t psm, BD_ADDR rem_bda, uint8_t info_type)
{
    tL2C_LCB        *p_lcb;
    tL2C_CCB        *p_ccb;
    tL2C_RCB        *p_rcb;
...
**
*******************************************************************************/
uint8_t L2CA_UcdSetIdleTimeout(BD_ADDR rem_bda, uint16_t timeout)
{
    tL2C_LCB        *p_lcb;
    tL2C_CCB        *p_ccb;
...
**
*******************************************************************************/
uint8_t L2CA_UCDSetTxPriority(BD_ADDR rem_bda, tL2CAP_CHNL_PRIORITY priority)
{
    tL2C_LCB        *p_lcb;
    tL2C_CCB        *p_ccb;
