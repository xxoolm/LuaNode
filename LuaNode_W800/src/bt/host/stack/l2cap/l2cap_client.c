...
struct l2cap_client_t
{
    ...
    uint16_t remote_mtu;
    uint8_t configured_self;
    uint8_t configured_peer;
...
static void disconnect_request_cb(uint16_t local_channel_id, uint8_t ack_required);
static void disconnect_completed_cb(uint16_t local_channel_id, uint16_t error_code);
static void congestion_cb(uint16_t local_channel_id, uint8_t is_congested);
...
l2cap_client_t *l2cap_client_new(const l2cap_client_callbacks_t *callbacks, void *context)
{
    ...
    l2cap_client_t *ret = (l2cap_client_t *)GKI_getbuf(sizeof(l2cap_client_t));
...
uint8_t l2cap_client_is_connected(const l2cap_client_t *client)
{
    assert(client != NULL);
    return client->local_channel_id != 0 && client->configured_self && client->configured_peer;
}

uint8_t l2cap_client_write(l2cap_client_t *client, buffer_t *packet)
{
    assert(client != NULL);
    assert(packet != NULL);
...
static void read_ready_cb(uint16_t local_channel_id, BT_HDR *packet)
{
    ...
    wm_memcpy(buffer_ptr(buffer), packet->data + packet->offset, packet->len);
    GKI_freebuf(packet);
