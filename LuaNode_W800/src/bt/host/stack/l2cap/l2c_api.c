...
uint16_t L2CA_ConnectLECocReq(uint16_t psm, BD_ADDR p_bd_addr, tL2CAP_LE_CFG_INFO *p_cfg)
{
    ...
    if(p_cfg)
    {
        wm_memcpy(&p_ccb->local_conn_cfg, p_cfg, sizeof(tL2CAP_LE_CFG_INFO));
    }
...
**
*******************************************************************************/
uint8_t L2CA_ConnectLECocRsp(BD_ADDR p_bd_addr, uint8_t id, uint16_t lcid, uint16_t result,
                             uint16_t status, tL2CAP_LE_CFG_INFO *p_cfg)
{
    ...
    else
    {
        ...
        wm_memcpy(conn_info.bd_addr, p_bd_addr, BD_ADDR_LEN);
...
**
*******************************************************************************/
uint8_t L2CA_GetPeerLECocConfig(uint16_t lcid, tL2CAP_LE_CFG_INFO *peer_cfg)
{
    ...
    if(peer_cfg != NULL)
    {
        wm_memcpy(peer_cfg, &p_ccb->peer_conn_cfg, sizeof(tL2CAP_LE_CFG_INFO));
    }
...
uint8_t L2CA_SetConnectionCallbacks(uint16_t local_cid, const tL2CAP_APPL_INFO *callbacks)
{
    ...
    {
        registration_control_block = (tL2C_RCB *)GKI_getbuf(sizeof(tL2C_RCB));
...
**
*******************************************************************************/
uint8_t L2CA_ConnectRsp(BD_ADDR p_bd_addr, uint8_t id, uint16_t lcid,
...
**
*******************************************************************************/
uint8_t L2CA_ErtmConnectRsp(BD_ADDR p_bd_addr, uint8_t id, uint16_t lcid, uint16_t result,
...
**
*******************************************************************************/
uint8_t L2CA_ConfigReq(uint16_t cid, tL2CAP_CFG_INFO *p_cfg)
{
    tL2C_CCB        *p_ccb;
...
**
*******************************************************************************/
uint8_t L2CA_ConfigRsp(uint16_t cid, tL2CAP_CFG_INFO *p_cfg)
{
    tL2C_CCB        *p_ccb;
...
**
*******************************************************************************/
uint8_t  L2CA_Ping(BD_ADDR p_bd_addr, tL2CA_ECHO_RSP_CB *p_callback)
{
    tL2C_LCB        *p_lcb;
    ...
    if(p_lcb->link_state == LST_CONNECTED)
    {
        ...
        btu_start_timer(&p_lcb->l2c_lcb_timer, BTU_TTYPE_L2CAP_LINK, L2CAP_ECHO_RSP_TIMEOUT_MS / 1000);
        #endif
    }
...
**
*******************************************************************************/
uint8_t  L2CA_Echo(BD_ADDR p_bd_addr, BT_HDR *p_data, tL2CA_ECHO_DATA_CB *p_callback)
{
    tL2C_LCB    *p_lcb;
    uint8_t       *pp;
...
**                  set up.
*******************************************************************************/
uint8_t L2CA_SetIdleTimeout(uint16_t cid, uint16_t timeout, uint8_t is_global)
{
    tL2C_CCB        *p_ccb;
    tL2C_LCB        *p_lcb;
...
**                  ACL link.
*******************************************************************************/
uint8_t L2CA_SetIdleTimeoutByBdAddr(BD_ADDR bd_addr, uint16_t timeout, tBT_TRANSPORT transport)
{
    tL2C_LCB        *p_lcb;
...
**
*******************************************************************************/
uint8_t L2CA_SetAclPriority(BD_ADDR bd_addr, uint8_t priority)
{
...
**
*******************************************************************************/
uint8_t L2CA_FlowControl(uint16_t cid, uint8_t data_enabled)
{
    tL2C_CCB  *p_ccb;
    uint8_t   on_off = !data_enabled;
...
**
*******************************************************************************/
uint8_t L2CA_SendTestSFrame(uint16_t cid, uint8_t sup_type, uint8_t back_track)
{
    tL2C_CCB        *p_ccb;
...
**
*******************************************************************************/
uint8_t L2CA_SetTxPriority(uint16_t cid, tL2CAP_CHNL_PRIORITY priority)
{
    tL2C_CCB        *p_ccb;
...
**
*******************************************************************************/
uint8_t L2CA_SetChnlDataRate(uint16_t cid, tL2CAP_CHNL_DATA_RATE tx, tL2CAP_CHNL_DATA_RATE rx)
{
    tL2C_CCB        *p_ccb;
...
**                  ACL link.
*******************************************************************************/
uint8_t L2CA_SetFlushTimeout(BD_ADDR bd_addr, uint16_t flush_tout)
{
    tL2C_LCB    *p_lcb;
    ...
    /* no retransmission */
        ...
        if(flush_tout == L2CAP_NO_RETRANSMISSION)
        {
        ...
        /* no automatic flush (infinite timeout) */
            ...
            if(flush_tout == L2CAP_NO_AUTOMATIC_FLUSH)
            {
                hci_flush_to = 0x0000;
            }
            else
...
**
*******************************************************************************/
uint8_t L2CA_GetPeerFeatures(BD_ADDR bd_addr, uint32_t *p_ext_feat, uint8_t *p_chnl_mask)
{
    tL2C_LCB        *p_lcb;
    ...
    *p_ext_feat = p_lcb->peer_ext_fea;
    wm_memcpy(p_chnl_mask, p_lcb->peer_chnl_mask, L2CAP_FIXED_CHNL_ARRAY_SIZE);
    return (TRUE);
}
...
**
*******************************************************************************/
uint8_t L2CA_GetBDAddrbyHandle(uint16_t handle, BD_ADDR bd_addr)
{
    tL2C_LCB *p_lcb = NULL;
    uint8_t found_dev = FALSE;
    p_lcb = l2cu_find_lcb_by_handle(handle);

    if(p_lcb)
    {
        found_dev = TRUE;
        wm_memcpy(bd_addr, p_lcb->remote_bd_addr, BD_ADDR_LEN);
    }
...
**
*******************************************************************************/
uint8_t L2CA_SetFixedChannelTout(BD_ADDR rem_bda, uint16_t fixed_cid, uint16_t idle_tout)
{
    tL2C_LCB        *p_lcb;
    tBT_TRANSPORT   transport = BT_TRANSPORT_BR_EDR;
    #if BLE_INCLUDED == TRUE
...
**
*******************************************************************************/
uint8_t L2CA_GetCurrentConfig(uint16_t lcid,
...
**
*******************************************************************************/
uint8_t L2CA_GetConnectionConfig(uint16_t lcid, uint16_t *mtu, uint16_t *rcid, uint16_t *handle)
{
    tL2C_CCB *p_ccb = l2cu_find_ccb_by_cid(NULL, lcid);;
    L2CAP_TRACE_API("%s CID: 0x%04x", __func__, lcid);
...
**
*******************************************************************************/
uint8_t L2CA_RegForNoCPEvt(tL2CA_NOCP_CB *p_cb, BD_ADDR p_bda)
{
    tL2C_LCB        *p_lcb;
...
**
*******************************************************************************/
uint8_t L2CA_SetChnlFlushability(uint16_t cid, uint8_t is_flushable)
{
