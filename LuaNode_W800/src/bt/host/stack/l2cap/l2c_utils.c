...
**
*******************************************************************************/
tL2C_LCB *l2cu_allocate_lcb(BD_ADDR p_bd_addr, uint8_t is_bonding, tBT_TRANSPORT transport)
{
    int         xx;
    ...
    for(xx = 0; xx < MAX_L2CAP_LINKS; xx++, p_lcb++)
    {
        if(!p_lcb->in_use)
        {
            ...
            #endif
            wm_memset(p_lcb, 0, sizeof(tL2C_LCB));
            wm_memcpy(p_lcb->remote_bd_addr, p_bd_addr, BD_ADDR_LEN);
            p_lcb->in_use          = TRUE;
            ...
            p_lcb->tx_data_len     = btm_cb.devcb.ble_packet_len;//controller_get_interface()->get_ble_default_data_packet_length();
...
**
*******************************************************************************/
void l2cu_update_lcb_4_bonding(BD_ADDR p_bd_addr, uint8_t is_bonding)
{
...
void l2cu_release_lcb(tL2C_LCB *p_lcb)
{
    ...
    GKI_free_and_reset_buf((void **)&p_lcb->p_hcit_rcv_acl);
...
**
** Returns          uint8_t - TRUE if cmd_code is a command and it is rejected,
**                            FALSE if response code. (command not rejected)
**
*******************************************************************************/
uint8_t l2c_is_cmd_rejected(uint8_t cmd_code, uint8_t id, tL2C_LCB *p_lcb)
{
    switch(cmd_code)
    {
                               ...
                               uint16_t p1, uint16_t p2)
{
    ...
    if(reason == L2CAP_CMD_REJ_MTU_EXCEEDED)
    {
        ...
        if(reason == L2CAP_CMD_REJ_INVALID_CID)
        {
            param_len = 4;
        }
        else
...
void l2cu_send_peer_config_rej(tL2C_CCB *p_ccb, uint8_t *p_data, uint16_t data_len, uint16_t rej_len)
{
    ...
    BT_HDR *p_buf = (BT_HDR *)GKI_getbuf(len + rej_len);
    p_buf->offset = L2CAP_SEND_CMD_OFFSET;
    p = (uint8_t *)(p_buf + 1) + L2CAP_SEND_CMD_OFFSET;
    /* Put in HCI header - handle + pkt boundary */
    #if (L2CAP_NON_FLUSHABLE_PB_INCLUDED == TRUE)
            ...
            default:
                ...
                if((cfg_len + L2CAP_CFG_OPTION_OVERHEAD) <= data_len)
                {
                    if((cfg_code & 0x80) == 0)
                    {
                        ...
                        {
                            wm_memcpy(p, p_data, cfg_len + L2CAP_CFG_OPTION_OVERHEAD);
...
void l2cu_send_peer_echo_rsp(tL2C_LCB *p_lcb, uint8_t id, uint8_t *p_data, uint16_t data_len)
{
    ...
    uint16_t acl_data_size = btu_cb.hcit_acl_data_size;
    uint16_t acl_packet_size = btu_cb.hcit_acl_pkt_size;
...
void l2cu_send_peer_info_req(tL2C_LCB *p_lcb, uint16_t info_type)
{
    ...
    #else
    p_lcb->info_resp_timer.p_cback = (TIMER_CBACK *)&l2c_info_resp_timer_timeout;
    p_lcb->info_resp_timer.param = (TIMER_PARAM_TYPE)p_lcb;
    btu_start_timer(&p_lcb->info_resp_timer, BTU_TTYPE_L2CAP_INFO,  L2CAP_WAIT_INFO_RSP_TIMEOUT_MS / 1000);
    #endif
    l2c_link_check_send_pkts(p_lcb, NULL, p_buf);
}
...
void l2cu_send_peer_info_rsp(tL2C_LCB *p_lcb, uint8_t remote_id, uint16_t info_type)
{
    ...
    #endif
    {
        ...
        {
            UINT16_TO_STREAM(p, L2CAP_INFO_RESP_RESULT_SUCCESS);
            wm_memset(p, 0, L2CAP_FIXED_CHNL_ARRAY_SIZE);
...
tL2C_CCB *l2cu_allocate_ccb(tL2C_LCB *p_lcb, uint16_t cid)
{
    ...
    wm_memset(&p_ccb->our_cfg, 0, sizeof(tL2CAP_CFG_INFO));
    wm_memset(&p_ccb->peer_cfg, 0, sizeof(tL2CAP_CFG_INFO));
    ...
    p_ccb->bypass_fcs = 0;
    wm_memset(&p_ccb->ertm_info, 0, sizeof(tL2CAP_ERTM_INFO));
...
**
** Returns          uint8_t  - TRUE if idle timer started or disconnect initiated
**                             FALSE if there's one or more pending CCB's exist
**
*******************************************************************************/
uint8_t l2cu_start_post_bond_timer(uint16_t handle)
{
            ...
            (p_lcb->link_state == LST_DISCONNECTING))
    {
        ...
        btu_start_timer(&p_lcb->l2c_lcb_timer, BTU_TTYPE_L2CAP_LINK, timeout_ms / 1000);
        #endif
        return (TRUE);
    }
...
void l2cu_release_ccb(tL2C_CCB *p_ccb)
{
    ...
    #endif
    fixed_queue_free(p_ccb->xmit_hold_q, GKI_freebuf);
...
{
    uint8_t  mtu_ok      = TRUE;
    uint8_t  qos_type_ok = TRUE;
    uint8_t  flush_to_ok = TRUE;
    uint8_t  fcr_ok      = TRUE;
    uint8_t    fcr_status;
    ...
    /* Reload flush_to from a previously accepted config request */
        ...
        if(p_ccb->peer_cfg.flush_to_present)
        {
...
void l2cu_process_our_cfg_req(tL2C_CCB *p_ccb, tL2CAP_CFG_INFO *p_cfg)
{
    ...
    if(p_cfg->fcr_present)
    {
        ...
        {
            wm_memset(&p_cfg->fcr, 0, sizeof(tL2CAP_FCR_OPTS));
        }
        else
        {
...
**
*******************************************************************************/
uint8_t l2cu_create_conn(tL2C_LCB *p_lcb, tBT_TRANSPORT transport)
{
    int             xx;
        ...
        if((p_lcb_cur->in_use) && (p_lcb_cur->link_role == HCI_ROLE_SLAVE))
        {
            ...
            if(HCI_SWITCH_SUPPORTED(BTM_ReadLocalFeatures()))
            {
                ...
                if(BTM_SwitchRole(p_lcb_cur->remote_bd_addr, HCI_ROLE_MASTER, NULL) == BTM_CMD_STARTED)
                {
                    ...
                    p_lcb->l2c_lcb_timer.p_cback = (TIMER_CBACK *)&l2c_lcb_timer_timeout;
                    p_lcb->l2c_lcb_timer.param = (TIMER_PARAM_TYPE)p_lcb;
                    btu_start_timer(&p_lcb->l2c_lcb_timer, BTU_TTYPE_L2CAP_LINK, L2CAP_LINK_ROLE_SWITCH_TIMEOUT_MS / 1000);
                    #endif
                    return (TRUE);
                }
            }
...
uint8_t l2cu_create_conn_after_switch(tL2C_LCB *p_lcb)
{
    ...
    p_lcb->l2c_lcb_timer.p_cback = (TIMER_CBACK *)&l2c_lcb_timer_timeout;
    p_lcb->l2c_lcb_timer.param = (TIMER_PARAM_TYPE)p_lcb;
    btu_start_timer(&p_lcb->l2c_lcb_timer, BTU_TTYPE_L2CAP_LINK, L2CAP_LINK_CONNECT_TIMEOUT_MS / 1000);
    #endif
    return (TRUE);
}
...
**
*******************************************************************************/
uint8_t l2cu_lcb_disconnecting(void)
{
    tL2C_LCB    *p_lcb;
    tL2C_CCB    *p_ccb;
    uint16_t      i;
    ...
    for(i = 0; i < MAX_L2CAP_LINKS; i++, p_lcb++)
    {
        if(p_lcb->in_use)
        {
            ...
            /* only one ccb left on lcb */
                ...
                if(p_lcb->ccb_queue.p_first_ccb == p_lcb->ccb_queue.p_last_ccb)
                {
...
uint8_t l2cu_set_acl_priority(BD_ADDR bd_addr, uint8_t priority, uint8_t reset_after_rs)
{
    tL2C_LCB            *p_lcb;
    uint8_t               *pp;
...
**
*******************************************************************************/
void l2cu_set_non_flushable_pbf(uint8_t is_supported)
{
    if(is_supported)
    {
...
void l2cu_no_dynamic_ccbs(tL2C_LCB *p_lcb)
{
    ...
    if(start_timeout)
    {
        L2CAP_TRACE_DEBUG("l2cu_no_dynamic_ccbs() starting IDLE timeout: %d", timeout_ms / 1000);
        p_lcb->l2c_lcb_timer.p_cback = (TIMER_CBACK *)&l2c_lcb_timer_timeout;
        p_lcb->l2c_lcb_timer.param = (TIMER_PARAM_TYPE)p_lcb;
        btu_start_timer(&p_lcb->l2c_lcb_timer, BTU_TTYPE_L2CAP_LINK, timeout_ms / 1000);
    }
...
void l2cu_process_fixed_disc_cback(tL2C_LCB *p_lcb)
{
    ...
    for(int xx = 0; xx < L2CAP_NUM_FIXED_CHNLS; xx++)
    {
        if(p_lcb->p_fixed_ccbs[xx])
        {
            ...
            if((peer_channel_mask & (1 << (xx + L2CAP_FIRST_FIXED_CHNL)))
...
void l2cu_set_acl_hci_header(BT_HDR *p_buf, tL2C_CCB *p_ccb)
{
    ...
    {
        UINT16_TO_STREAM(p, p_ccb->p_lcb->handle | (L2CAP_PKT_START_NON_FLUSHABLE << L2CAP_PKT_TYPE_SHIFT));
        uint16_t acl_data_size = btu_cb.hcit_ble_acl_data_size;
        ...
        if(p_ccb->cong_sent)
        {
            ...
            if(q_count <= (p_ccb->buff_quota / 2))
            {
                ...
                if(p_ccb->p_rcb && p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb)
                {
                    ...
                    if(p_ccb->p_rcb && p_ccb->local_cid == L2CAP_CONNECTIONLESS_CID)
                    {
