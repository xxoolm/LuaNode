...
static void cmac_aes_cleanup(void)
{
    ...
    wm_memset(&cmac_cb, 0, sizeof(tCMAC_CB));
}
...
**
*******************************************************************************/
static uint8_t cmac_aes_k_calculate(BT_OCTET16 key, uint8_t *p_signature, uint16_t tlen)
{
    tSMP_ENC output;
    uint8_t    i = 1, err = 0;
    uint8_t    x[16] = {0};
    ...
    while(i <= cmac_cb.round)
    {
        ...
        wm_memcpy(x, output.param_buf, BT_OCTET16_LEN);
        i ++;
    }
...
**
*******************************************************************************/
static uint8_t cmac_generate_subkey(BT_OCTET16 key)
{
    BT_OCTET16 z = {0};
    uint8_t     ret = TRUE;
    tSMP_ENC output;
...
**
*******************************************************************************/
uint8_t aes_cipher_msg_auth_code(BT_OCTET16 key, uint8_t *input, uint16_t length,
                                 uint16_t tlen, uint8_t *p_signature)
{
    ...
    cmac_cb.text = (uint8_t *)GKI_getbuf(len);
    cmac_cb.round = n;
    diff = len - length;
