...
void smp_send_app_cback(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    #if DEBUG_SMP_FUNCTION == 1
    SMP_TRACE_DEBUG(">>>%s evt=[%s][%d]\r\n", __func__, smp_cb_evt_code_2_str(p_cb->cb_evt),  p_cb->cb_evt);
    #endif
        ...
        if(callback_rc == SMP_SUCCESS)
        {
            switch(p_cb->cb_evt)
            {
                case SMP_IO_CAP_REQ_EVT:
                    ...
                    SMP_TRACE_DEBUG(">>>callback_rc=%d  p_cb->cb_evt=%d, auth_req=%d", callback_rc, p_cb->cb_evt,cb_data.io_req.auth_req);
