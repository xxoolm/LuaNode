...
static uint8_t smp_calculate_legacy_short_term_key(tSMP_CB *p_cb, tSMP_ENC *output);
...
void smp_debug_print_nbyte_little_endian(uint8_t *p, const uint8_t *key_name, uint8_t len)
{
    ...
    wm_memset(p_buf, 0, sizeof(p_buf));
...
**
*******************************************************************************/
uint8_t smp_encrypt_data(uint8_t *key, uint8_t key_len,
                         ...
                         tSMP_ENC *p_out)
{
    ...
    p_start = (uint8_t *)GKI_getbuf(SMP_ENCRYT_DATA_SIZE * 4);
...
void smp_generate_stk(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    if(p_cb->le_secure_connections_mode_is_used)
    {
        ...
        wm_memcpy(output.param_buf, p_cb->ltk, SMP_ENCRYT_DATA_SIZE);
    }
    else
        if(!smp_calculate_legacy_short_term_key(p_cb, &output))
        {
            SMP_TRACE_ERROR("%s failed", __func__);
...
void smp_compute_csrk(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    {
        wm_memcpy((void *)p_cb->csrk, output.param_buf, BT_OCTET16_LEN);
...
void smp_gen_p2_4_confirm(tSMP_CB *p_cb, BT_OCTET16 p2)
{
    ...
    wm_memset(p, 0, sizeof(BT_OCTET16));
...
static void smp_process_confirm(tSMP_CB *p_cb, tSMP_ENC *p)
{
    tSMP_KEY    key;
    SMP_TRACE_DEBUG("%s", __FUNCTION__);
    wm_memcpy(p_cb->confirm, p->param_buf, BT_OCTET16_LEN);
    #if (SMP_DEBUG == TRUE)
...
static void smp_generate_ltk_cont(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    else
    {
        ...
        wm_memcpy((void *)p_cb->ltk, output.param_buf, BT_OCTET16_LEN);
...
**
*******************************************************************************/
uint8_t smp_calculate_legacy_short_term_key(tSMP_CB *p_cb, tSMP_ENC *output)
{
    BT_OCTET16 ptext;
    uint8_t *p = ptext;
    SMP_TRACE_DEBUG("%s", __func__);
    wm_memset(p, 0, BT_OCTET16_LEN);

    if(p_cb->role == HCI_ROLE_MASTER)
    {
        wm_memcpy(p, p_cb->rand, BT_OCTET8_LEN);
        wm_memcpy(&p[BT_OCTET8_LEN], p_cb->rrand, BT_OCTET8_LEN);
    }
    else
    {
        wm_memcpy(p, p_cb->rrand, BT_OCTET8_LEN);
        wm_memcpy(&p[BT_OCTET8_LEN], p_cb->rand, BT_OCTET8_LEN);
    }
...
void smp_use_oob_private_key(tSMP_CB *p_cb, tSMP_INT_DATA *p_data)
{
    ...
    switch(p_cb->req_oob_type)
    {
        ...
        case SMP_OOB_LOCAL:
            ...
            wm_memcpy(p_cb->private_key, p_cb->sc_oob_data.loc_oob_data.private_key_used, BT_OCTET32_LEN);
...
void smp_continue_private_key_creation(tSMP_CB *p_cb, tBTM_RAND_ENC *p)
{
    ...
    switch(state)
    {
        case SMP_GENERATE_PRIVATE_KEY_0_7:
            wm_memcpy((void *)p_cb->private_key, p->param_buf, p->param_len);
        ...
        case SMP_GENERATE_PRIVATE_KEY_8_15:
            wm_memcpy((void *)&p_cb->private_key[8], p->param_buf, p->param_len);
        ...
        case SMP_GENERATE_PRIVATE_KEY_16_23:
            wm_memcpy((void *)&p_cb->private_key[16], p->param_buf, p->param_len);
        ...
        case SMP_GENERATE_PRIVATE_KEY_24_31:
            wm_memcpy((void *)&p_cb->private_key[24], p->param_buf, p->param_len);
            smp_process_private_key(p_cb);
            break;
...
void smp_process_private_key(tSMP_CB *p_cb)
{
    ...
    SMP_TRACE_DEBUG("%s", __FUNCTION__);
    wm_memcpy(private_key, p_cb->private_key, BT_OCTET32_LEN);
    ECC_PointMult(&public_key, &(curve_p256.G), (DWORD *) private_key, KEY_LENGTH_DWORDS_P256);
    wm_memcpy(p_cb->loc_publ_key.x, public_key.x, BT_OCTET32_LEN);
    wm_memcpy(p_cb->loc_publ_key.y, public_key.y, BT_OCTET32_LEN);
...
void smp_compute_dhkey(tSMP_CB *p_cb)
{
    ...
    SMP_TRACE_DEBUG("%s", __FUNCTION__);
    wm_memcpy(private_key, p_cb->private_key, BT_OCTET32_LEN);
    wm_memcpy(peer_publ_key.x, p_cb->peer_publ_key.x, BT_OCTET32_LEN);
    wm_memcpy(peer_publ_key.y, p_cb->peer_publ_key.y, BT_OCTET32_LEN);
    ECC_PointMult(&new_publ_key, &peer_publ_key, (DWORD *) private_key, KEY_LENGTH_DWORDS_P256);
    wm_memcpy(p_cb->dhkey, new_publ_key.x, BT_OCTET32_LEN);
...
**
*******************************************************************************/
uint8_t smp_calculate_f5(uint8_t *w, uint8_t *n1, uint8_t *n2, uint8_t *a1, uint8_t *a2,
...
**
*******************************************************************************/
uint8_t smp_calculate_f5_mackey_or_long_term_key(uint8_t *t, uint8_t *counter,
...
**
*******************************************************************************/
uint8_t smp_calculate_f5_key(uint8_t *w, uint8_t *t)
{
    uint8_t *p = NULL;
...
**
*******************************************************************************/
uint8_t smp_calculate_f6(uint8_t *w, uint8_t *n1, uint8_t *n2, uint8_t *r, uint8_t *iocap, uint8_t *a1,
...
**
*******************************************************************************/
uint8_t smp_calculate_link_key_from_long_term_key(tSMP_CB *p_cb)
{
    ...
    if(p_cb->id_addr_rcvd && p_cb->id_addr_type == BLE_ADDR_PUBLIC)
    {
        ...
        wm_memcpy(bda_for_lk, p_cb->id_addr, BD_ADDR_LEN);
    }
    else
        if((BTM_ReadRemoteConnectionAddr(p_cb->pairing_bda, bda_for_lk, &conn_addr_type)) &&
...
**
*******************************************************************************/
uint8_t smp_calculate_long_term_key_from_link_key(tSMP_CB *p_cb)
{
    uint8_t ret = TRUE;
    tBTM_SEC_DEV_REC *p_dev_rec;
...
**
*******************************************************************************/
uint8_t smp_calculate_h6(uint8_t *w, uint8_t *keyid, uint8_t *c)
{
    #if SMP_DEBUG == TRUE
    uint8_t   *p_print = NULL;
    #endif
    ...
    if(p && p->status == HCI_SUCCESS)
    {
        switch(state)
        {
            case SMP_GEN_SRAND_MRAND:
                wm_memcpy((void *)p_cb->rand, p->param_buf, p->param_len);
                smp_generate_rand_cont(p_cb, NULL);
                break;

            case SMP_GEN_SRAND_MRAND_CONT:
                wm_memcpy((void *)&p_cb->rand[8], p->param_buf, p->param_len);
            ...
            case SMP_GEN_RAND_V:
                wm_memcpy(p_cb->enc_rand, p->param_buf, BT_OCTET8_LEN);
