...
#if SMP_DYNAMIC_MEMORY == TRUE
	tSMP_CB *smp_cb_ptr = NULL;
#endif
...
{
#if SMP_DYNAMIC_MEMORY == TRUE
	smp_cb_ptr = (tSMP_CB*)GKI_os_malloc(sizeof(tSMP_CB));
    assert(smp_cb_ptr != NULL);
#endif
    wm_memset(&smp_cb, 0, sizeof(tSMP_CB));
...
**
*******************************************************************************/
uint8_t SMP_Register(tSMP_CALLBACK *p_cback)
{
...
tSMP_STATUS SMP_Pair(BD_ADDR bd_addr)
{
    ...
    else
    {
        ...
        wm_memcpy(p_cb->pairing_bda, bd_addr, BD_ADDR_LEN);
...
**
*******************************************************************************/
uint8_t SMP_PairCancel(BD_ADDR bd_addr)
{
    tSMP_CB   *p_cb = &smp_cb;
...
*******************************************************************************/
uint8_t SMP_Encrypt(uint8_t *key, uint8_t key_len,
...
*******************************************************************************/
uint8_t SMP_CreateLocalSecureConnectionsOobData(tBLE_BD_ADDR *addr_to_send_to)
{
    tSMP_CB *p_cb = &smp_cb;
    uint8_t   *bd_addr;
