    ...
    switch(event)
    {
        ...
        case BTA_HF_CLIENT_OPEN_EVT:
            if(p_data->open.status == BTA_HF_CLIENT_SUCCESS)
            {
                ...
                if(btif_hf_client_cb.state == BTHF_CLIENT_CONNECTION_STATE_CONNECTING)
                {
        ...
        case BTA_HF_CLIENT_CLCC_EVT:
			...
			msg.current_calls_msg.dir = p_data->clcc.inc ? BTHF_CLIENT_CALL_DIRECTION_INCOMING : BTHF_CLIENT_CALL_DIRECTION_OUTGOING;
			...
			msg.current_calls_msg.mpty = p_data->clcc.mpty ? BTHF_CLIENT_CALL_MPTY_TYPE_MULTI : BTHF_CLIENT_CALL_MPTY_TYPE_SINGLE;
