...
tls_os_sem_t *bdroid_sem = NULL;
static tls_os_queue_t *host_msg_queue = NULL;
#define ACTIVE_HOST_TASK  1
#define HOST_QUEUE_SIZE 10

void bdroid_timer_cb(void *ptmr, void *parg)
{
    GKI_timer_update(1);
}
void active_host_task()
{
#if 0
    if(bdroid_sem == NULL)
    {
    ...
    tls_os_sem_release(bdroid_sem);
#else
    if(host_msg_queue == NULL)
    {
        return;
    }
    tls_os_queue_send(host_msg_queue, (void*)ACTIVE_HOST_TASK, 0);
#endif
}
...
void bt_host_task()
{
    ...
    {
    #if 0
        err = tls_os_sem_acquire(bdroid_sem, 0);
    ...
    #else
        tls_os_queue_receive(host_msg_queue, (void **)&msg, 0, 0);
        switch((u32)msg)
		{
			case ACTIVE_HOST_TASK:
			    GKI_run(NULL);
				break;
...
tls_os_status_t wm_bt_host_init(void)
{
	tls_os_status_t err;
    #if 0
    err = tls_os_sem_create(&bdroid_sem, 0);
    #else
...
tls_bt_status_t tls_bt_get_remote_device_property(tls_bt_addr_t *remote_addr, tls_bt_property_type_t type)
{
    /* sanity check */
    if(interface_ready() == FALSE)
    {
...
tls_bt_status_t tls_bt_set_remote_device_property(tls_bt_addr_t *remote_addr, const tls_bt_property_t *property)
{
    /* sanity check */
    if(interface_ready() == FALSE)
    {
