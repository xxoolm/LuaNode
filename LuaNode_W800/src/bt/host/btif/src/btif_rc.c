...
static tls_bt_status_t register_notification_cmd(uint8_t label, uint8_t event_id, uint32_t event_value);
    ...
    switch(event)
    {
        ...
        case BTA_AV_META_MSG_EVT:
        {
            if(bt_rc_callbacks != NULL)
            {
                ...
                if((bt_rc_callbacks == NULL) && (bt_rc_ctrl_callbacks != NULL))
                {
                            ...
                            (p_data->meta_msg.code <= AVRC_RSP_INTERIM))
                    {
                        ...
                        if(p_data->meta_msg.code <= AVRC_CMD_GEN_INQ)
                        {
                            /* Its a command  */
    ...
    switch(event)
    {
        ...
        case AVRC_PDU_GET_ELEMENT_ATTR:
        {
            ...
            if(pavrc_cmd->get_elem_attrs.num_attr == 0)
            {
                ...
                if(pavrc_cmd->get_elem_attrs.num_attr == 0xFF)
                {
			...
			msg.get_element_attr.p_attrs = element_attrs;
            ...
            || AVRC_RSP_REJ == pmeta_msg->code || AVRC_RSP_NOT_IMPL == pmeta_msg->code))
    {
        ...
        if(status != AVRC_STS_NO_ERROR)
        {
                    ...
                    && btif_rc_cb.rc_vol_label == pmeta_msg->label)
            {
                ...
                if(AVRC_PDU_SET_ABSOLUTE_VOLUME == avrc_response.rsp.pdu)
                {
...
static void rc_start_play_status_timer(void)
{
    ...
    if(!btif_rc_cb.rc_play_status_timer.in_use)
    {
        ...
        btu_start_timer_oneshot(&btif_rc_cb.rc_play_status_timer, BTU_TTYPE_USER_FUNC, BTIF_TIMEOUT_RC_INTERIM_RSP_MS / 1000);
    }
...
static void register_for_event_notification(btif_rc_supported_event_t *p_event)
{
    ...
    if(status == TLS_BT_STATUS_SUCCESS)
    {
        ...
        btu_start_timer_oneshot(&p_transaction->txn_timer, BTU_TTYPE_USER_FUNC, BTIF_TIMEOUT_RC_INTERIM_RSP_MS / 1000);
        #endif
    }
    else
    {
...
static void start_status_command_timer(uint8_t pdu_id, rc_transaction_t *p_txn)
{
    ...
    btu_start_timer_oneshot(&p_txn->txn_timer, BTU_TTYPE_USER_FUNC, BTIF_TIMEOUT_RC_STATUS_CMD_MS / 1000);
    #endif
}
...
static void start_control_command_timer(uint8_t pdu_id, rc_transaction_t *p_txn)
{
    ...
    btu_start_timer_oneshot(&p_txn->txn_timer, BTU_TTYPE_USER_FUNC, BTIF_TIMEOUT_RC_CONTROL_CMD_MS / 1000);
    #endif
}
...
static void handle_app_val_response(tBTA_AV_META_MSG *pmeta_msg, tAVRC_LIST_APP_VALUES_RSP *p_rsp)
{
    ...
    if(p_app_settings->attr_index < p_app_settings->num_attrs)
    {
        ...
        if(attr_index < p_app_settings->num_attrs)
        {
            ...
            if(p_app_settings->ext_attr_index < p_app_settings->num_ext_attrs)
            {
                attr_index = 0;
            ...
            else
            {
				...
				msg.playerapplicationsetting.app_attrs = p_app_settings->attrs;
...
static void handle_app_cur_val_response(tBTA_AV_META_MSG *pmeta_msg, tAVRC_GET_CUR_APP_VALUE_RSP *p_rsp)
{
    ...
    msg.playerapplicationsetting_changed.bd_addr = &rc_addr;
	msg.playerapplicationsetting_changed.p_vals = &app_settings;
...
static void handle_app_attr_val_txt_response(tBTA_AV_META_MSG *pmeta_msg, tAVRC_GET_APP_ATTR_TXT_RSP *p_rsp)
{
    ...
    else
    {
		...
		msg.playerapplicationsetting.ext_attrs = (tls_btrc_player_app_ext_attr_t *)&p_app_settings->ext_attrs[0];
		if(bt_rc_ctrl_callbacks) bt_rc_ctrl_callbacks(WM_BTRC_CTRL_PLAYERAPPLICATIONSETTING_EVT, &msg);
        ...
        tAVRC_GET_ELEM_ATTRS_RSP *p_rsp)
{
    ...
    if(p_rsp->status == AVRC_STS_NO_ERROR)
    {
		...
		msg.track_changed.p_attrs = p_attr;
        ...
        if(p_rsp->status == BTIF_RC_STS_TIMEOUT)
        {
            ...
            (pmeta_msg->code <= AVRC_CMD_GEN_INQ))
    {
        ...
        else
        {
            if(avrc_cmd.pdu == AVRC_PDU_REGISTER_NOTIFICATION)
            {
                ...
                if(avrc_cmd.pdu == AVRC_PDU_SET_ABSOLUTE_VOLUME)
                {
...
tls_bt_status_t tls_btrc_ctrl_volume_change_notification_rsp(tls_bt_addr_t *bd_addr, btrc_notification_type_t rsp_type,
