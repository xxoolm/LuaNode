                                     ...
                                     tls_bt_property_t *properties)
{
    ...
    for(i = 0; i < num_properties; i++)
    {
        ...
        if(properties[i].type == WM_BT_PROPERTY_BDADDR)
        {
            ...
            TLS_BT_APPL_TRACE_DEBUG("\t\t%02x:%02x:%02x:%02x:%02x:%02x\r\n", p_value[0],p_value[1],p_value[2],p_value[3],p_value[4],p_value[5]);
        }
	...
	switch(evt)
	{
	   ...
	   case WM_BT_DEVICE_FOUND_EVT:
		...
		case WM_BT_ACL_STATE_CHG_EVT:
			app_acl_state_changed_callback(msg->acl_state.status, msg->acl_state.remote_address, msg->acl_state.state,msg->acl_state.link_type);
			break;
...
tls_bt_status_t wm_bt_deregister_report_evt(tls_bt_host_evt_t rpt_evt,  tls_bt_host_callback_t rpt_callback)
{
	uint32_t cpu_sr;
	host_report_evt_t *evt = NULL;
	host_report_evt_t *evt_next = NULL;
