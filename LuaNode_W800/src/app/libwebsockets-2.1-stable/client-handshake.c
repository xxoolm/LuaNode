...
lws_client_connect_2(struct lws *wsi)
{
...
#endif
	{
		...
		server_addr4.sin_addr.s_addr = ((struct in_addr *)p)->s_addr;
...
#else
	ret = connect(wsi->sock, v, n);
	ret = Check_Connect_And_Wait_Connection(wsi->sock, ret);
	if (ret) {
		goto failed;
	}
#endif

	lwsl_client("sock %d connected\n", wsi->sock);
