...
#if defined(LWS_USE_POLARSSL)
	n = HTTPWrapperSSLConnect(&wsi->ssl, wsi->sock, NULL, 0, NULL);
#else
#if defined(LWS_USE_MBEDTLS)
#else
...
some_wait:
	...
	if (n <= 0) {
		...
		if(n  < 0)
		{
			...
			lwsl_info("SSL connect success %d\n", n);
#else
#if defined(LWS_USE_MBEDTLS)
#else
		n = ERR_get_error();
...
lws_ssl_client_connect2(struct lws *wsi)
{
	...
	if (wsi->mode == LWSCM_WSCL_WAITING_SSL) {
		lws_latency_pre(context, wsi);
#if defined(LWS_USE_POLARSSL)
	n = HTTPWrapperSSLConnect(&ssl_p, wsi->sock, NULL, 0, NULL);
	wsi->ssl = ssl_p;
#else
#if defined(LWS_USE_MBEDTLS)
#else
		...
		if (n <= 0) {
			...
			if(n  < 0)
			{
				lwsl_err("SSL connect 2 error %d\n", n);
				if(ssl_p)
				{
					HTTPWrapperSSLClose(ssl_p, wsi->sock);
					wsi->ssl = NULL;
				}
				return -1;
			}
