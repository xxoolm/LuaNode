...
#if defined(LWS_USE_POLARSSL)
	n = HTTPWrapperSSLRecv(wsi->ssl, wsi->sock, (char *)buf, len, 0);
...
#if defined(LWS_USE_POLARSSL)
	n = HTTPWrapperSSLSend(wsi->ssl, wsi->sock, (char *)buf, len, 0);
#else
#if defined(LWS_USE_MBEDTLS)
#else
...
lws_server_socket_service_ssl(struct lws *wsi, lws_sockfd_type accept_fd)
{
...
#if defined(LWS_USE_POLARSSL)
	{
		...
		rc = ssl_init((ssl_context *)wsi->ssl);
		if (rc) {
			lwsl_err("ssl_init failed\n");
			goto fail;
		}

		ssl_set_endpoint((ssl_context *)wsi->ssl, SSL_IS_SERVER);
		ssl_set_authmode((ssl_context *)wsi->ssl, SSL_VERIFY_OPTIONAL);
		ssl_set_rng((ssl_context *)wsi->ssl, urandom_bytes, NULL);
		ssl_set_dbg((ssl_context *)wsi->ssl, pssl_debug, NULL);
		ssl_set_bio((ssl_context *)wsi->ssl, net_recv, &wsi->sock, net_send, &wsi->sock);

		ssl_set_ciphersuites((ssl_context *)wsi->ssl, (int *)ciphers);

		ssl_set_session((ssl_context *)wsi->ssl, 1, 600, ssn);

		ssl_set_ca_chain((ssl_context *)wsi->ssl, &wsi->vhost->ssl_ctx->ca,
				 NULL, NULL);
		...
		if (wsi->vhost->allow_non_ssl_on_ssl_port) {
			if (n >= 1 && pt->serv_buf[0] >= ' ') {
				...
				*/
				wsi->use_ssl = 0;
#if defined(LWS_USE_POLARSSL)
				ssl_close_notify((ssl_context *)wsi->ssl);
				ssl_free((ssl_context *)wsi->ssl);
#else
#if defined(LWS_USE_MBEDTLS)
#else
				SSL_shutdown(wsi->ssl);
...
#if defined(LWS_USE_POLARSSL)
		n = ssl_handshake((ssl_context *)wsi->ssl);
#else
#if defined(LWS_USE_MBEDTLS)
#else
