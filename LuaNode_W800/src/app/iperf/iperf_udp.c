...
iperf_udp_recv(struct iperf_stream *sp)
{
    ...
    /* jitter measurement */
    iperf_gettimeofday(&arrival_time, NULL);
...
iperf_udp_connect(struct iperf_test *test)
{
    ...
    if ((s = netdial(test->settings->domain, Pudp, test->bind_address, test->server_hostname, test->server_port, test->local_port+100)) < 0) {
