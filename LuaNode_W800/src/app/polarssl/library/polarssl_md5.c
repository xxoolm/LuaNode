...
#if !WM_OPT_PORT
extern void MD5Init(md5_context *ctx);
extern void MD5Transform(u32 buf[4], u32 const in[16]);
extern void MD5Update(md5_context *ctx, unsigned char const *buf, unsigned len);
extern void MD5Final(unsigned char digest[16], md5_context *ctx);
extern int md5(const u8 *addr, int len, u8 *mac);
extern int hmac_md5(const u8 *key, size_t key_len, const u8 *data, size_t data_len,
	      u8 *mac);
#endif
               ...
               unsigned char output[16] )
{
    ...
    memset( &ctx, 0, sizeof( md5_context ) );
#else
    hmac_md5(key, keylen, input, ilen, output);
#endif
}
...
int md5_self_test( int verbose )
{
    ...
    for( i = 0; i < 7; i++ )
    {
        ...
        p_md5( md5_test_buf[i], md5_test_buflen[i], md5sum );
